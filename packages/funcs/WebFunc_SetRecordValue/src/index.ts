...
const main = function (tableName:string, index:string, fields:string, values:string) {
  ...
  if (index == null || index == '')
    throw vds.exception.newConfigException(
      '函数[SetRecordValue]：第二个参数不能为空'
    )
  if (fields == null)
    throw vds.exception.newConfigException(
      '函数[SetRecordValue]：第三个参数不能为空'
    )
  if (values == null)
    throw vds.exception.newConfigException(
      '函数[SetRecordValue]：第四个参数不能为空'
    )
