...
const main = function (templateStr: string) {
  ...
  let tableNamesMap: Record<string, any> = {}
  // 提取所有的数据源tableNames
  if (arr && arr.length > 0) {
      ...
      if (sourceStr.indexOf('.') != -1) {
        ...
        if (matchs && matchs.length > 1) {
          ...
          if (tableName != '') {
            ...
            if (!tableNamesMap[tableName]) {
              // 之前已经获取过数据了就不重复获取
              // let tableName = tableNamesMap[key];
              let datasource = vds.ds.lookup(tableName)
              if (null == datasource) {
        ...
        if (otherVar) {
          try {
            ...
            let value = vds.expression.execute(expressionTemp)
            let value2 = vds.expression.execute(expressionTemp2)
  ...
  // 用模板引擎进行替换值
  let content = vds.string.template(templateStr, data)
