...
const main = function (param) {
  ...
  if (math.isEmpty(arg)) throw new Error('双曲正弦值函数参数为空，请检查')
  if (!math.judgeNum(arg)) throw new Error('双曲正弦值函数参数不是数字，请检查')
  ...
  let ret = math.subtract(num1, num2)
  let result = math.divide(ret, 2, 10)
