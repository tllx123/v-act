...
prompts({
  ...
  if (appPackage.devDependencies) {
    // We used to put react-v3-scripts in devDependencies
  ...
  if (fs.existsSync(paths.appTypeDeclarations)) {
    try {
      ...
      // Remove react-v3-scripts reference since they're getting a copy of the types in their project
      content =
        content
          // Remove react-v3-scripts types
          .replace(
            /^\s*\/\/\/\s*<reference\s+types.+?"react-v3-scripts".*\/>.*(?:\n|$)/gm,
            ''
          )
          .trim() + os.EOL;
  ...
  if (ownPath.indexOf(appPath) === 0) {
    try {
      // remove react-v3-scripts and react-v3-scripts binaries from app node_modules
      Object.keys(ownPackage.bin).forEach(binKey => {
  ...
  if (fs.existsSync(paths.yarnLockFile)) {
    const windowsCmdFilePath = path.join(
      ...
      'node_modules',
      '.bin',
      'react-v3-scripts.cmd'
    );
    let windowsCmdFileContent;
    if (process.platform === 'win32') {
      // https://github.com/v3/create-react-app/pull/3806#issuecomment-357781035
      // Yarn is diligent about cleaning up after itself, but this causes the react-v3-scripts.cmd file
