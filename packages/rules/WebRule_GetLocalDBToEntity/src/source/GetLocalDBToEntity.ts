...
let main = function (ruleContext) {
  ...
  let isLocalDb = inParamsObj['isLocalDb']
    ...
    let asyFun = function (itemConfig) {
      ...
      if (
        ...
        for (let i = 0; i < queryConds.length; i++) {
          let cond = queryConds[i]
          if (
            cond.operation != 'is' &&
            cond.operation != 'is not' &&
            cond.valueType == 9
          ) {
            let calValue = engine.execute({
...
let removeNullCondition = function (queryConds) {
  let result = []
  if (undefined != queryConds && null != queryConds && queryConds.length > 0) {
    for (let i = 0; i < queryConds.length; i++) {
      ...
      let valueObj = queryConditionUtil.getCondRight(
        cond['value'],
        cond['valueType'],
        cond['columnType'],
        cond['operation'],
