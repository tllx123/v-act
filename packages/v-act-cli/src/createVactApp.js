...
let globalDefaultDependenciesVersion = {
  "react":null,
  "react-dom":null,
  "@v-act/scripts":null
}
function initDefaultDependenciesVersion(){
  const promise = new Promise((resolve, reject)=>{
    let pros = [];
    const depNames = Object.keys(globalDefaultDependenciesVersion);

    console.log(`start check dependencies version: ${depNames}`);

    Object.keys(globalDefaultDependenciesVersion).forEach((name)=>{
      const pro = npmUtils.getVersion(name);
      pro.then((version)=>{
        globalDefaultDependenciesVersion[name] = version;
      })
      pros.push(pro);
    });
    Promise.all(pros).then(()=>{
      console.log("dependencies version check finish.");
      resolve();
    }).catch(reject);
  });
  return promise;
...
function init() {
  const program = new commander.Command(packageJson.name)
    ...
    .option(
      ...
      'use a non-standard version of @v-act/scripts'
    )
    .option(
...
function createApp(name, verbose, version, template, useYarn, usePnp) {
  ...
  if (unsupportedNodeVersion) {
    ...
    // Fall back to latest supported @v-act/scripts on Node 4
    version = '@v-act/scripts@0.9.x';
  }
  ...
  if (!useYarn) {
    ...
    if (!npmInfo.hasMinNpm) {
      ...
      // Fall back to latest supported @v-act/scripts for npm 3
      version = '@v-act/scripts@0.9.x';
    }
  } else if (usePnp) {
...
function install(root, useYarn, usePnp, dependencies, verbose, isOnline) {
  return new Promise((resolve, reject) => {
    ...
    //如果依赖能统一从父级获取，则能大幅提升创建的耗时（几百秒-->几十秒），但创建完成后，需自行install
    const allDependencies = ['react', 'react-dom', '@v-act/scripts'];
    args = args.filter((val)=>{
      return (allDependencies.indexOf(val) == -1);
    })
  ...
  Promise.all([
    ...
    Promise.all([
      ...
      .then(({ isOnline, packageInfo, templateInfo }) => {
        ...
        // Only support templates when used alongside new @v-act/scripts versions.
        ...
        if (supportsTemplates) {
          ...
          console.log(
            `The ${chalk.cyan(packageInfo.name)} version you're using ${
              packageInfo.name === '@v-act/scripts' ? 'is not' : 'may not be'
        ...
        //   }...`
        // );
        console.log(`Installing ${chalk.cyan(templateInfo.name)}`);
        console.log();
        // console.log([root, appName, verbose, originalDirectory, templateInfo.name].join("@"));
        return install(
          root,
          useYarn,
          usePnp,
      ...
      .then(async ({ packageInfo, supportsTemplates, templateInfo }) => {
        console.log("install完成，耗时：" + (new Date().getTime() - globalTime));
      ...
      //   );
        ...
        if (version === '@v-act/scripts@0.9.x') {
          console.log(
            chalk.yellow(
...
function getInstallPackage(version, originalDirectory) {
  let packageToInstall = '@v-act/scripts';
...
function getPackageInfo(installPackage) {
  if (installPackage.match(/^.+\.(tgz|tar\.gz)$/)) {
    return getTemporaryDirectory()
      ...
      .catch(err => {
        // The package name could be with or without semver version, e.g. @v-act/scripts-0.2.0-alpha.1.tgz
    ...
    // git+https://github.com/mycompany/@v-act/scripts.git
    // git+ssh://github.com/mycompany/@v-act/scripts.git#v1.2.3
...
function checkAppName(appName) {
  ...
  const dependencies = ['react', 'react-dom', '@v-act/scripts'].sort();
...
function setCaretRangeForRuntimeDeps(packageName) {
  ...
  for (const name in globalDefaultDependenciesVersion) {
    if (Object.hasOwnProperty.call(globalDefaultDependenciesVersion, name)) {
      deps[name] = globalDefaultDependenciesVersion[name];
    }
  }
  ...
  makeCaretRange(deps, 'react');
  makeCaretRange(deps, 'react-dom');
...
function executeChildPlugin(childScriptPath, data){
  return new Promise((resolve, reject)=>{
    let child_pro = childProcess.fork(childScriptPath, [JSON.stringify(data)],{
        cwd: process.cwd(),
        silent: false
    });
    child_pro.on("close",()=>{
