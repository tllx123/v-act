define('./TreeHandler', function (require, exports, module) {
  ...
  export function initModule(sb) {
    ...
    treeViewUtil = sb.getService(
      'vjs.framework.extension.platform.services.domain.tree.TreeViewUtil'
    )
    treeDataTranslator = sb.getService(
      'vjs.framework.extension.platform.services.domain.tree.TreeDataTranslator'
    )
    widgetAction = sb.getService(
      'vjs.framework.extension.platform.services.view.widget.common.action.WidgetAction'
    )
    treeViewUIHelp = sb.getService(
      'vjs.framework.extension.platform.services.viewmodel.datamodel.tree.TreeViewUtil'
    )
    treeViewModelFactory = sb.getService(
      'vjs.framework.extension.platform.services.viewmodel.datamodel.tree.TreeDataModelFactory'
    )
    ExpressionContext = sb.getService(
      'vjs.framework.extension.platform.engine.expression.ExpressionContext'
    )
    formulaEngine = sb.getService(
  ...
  export function genInsertAction(widgetCode) {
    return function (tempParams) {
      ...
      if (
        widget.isRowNumberField(widget.fields[0]) &&
        widget.resetRowNumberField
      )
