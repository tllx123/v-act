exports.initModule = function (sBox) {
  ...
  //>	@class	JGFormItemModel
  //
  // The JGFormItemModel manages data from multiple member forms.
  // <P>
  ...
  // JGFormItemModel can be used to make the forms act as one logical form, supporting all
  // value-related APIs otherwise called on DynamicForm directly.
  // <P>
  // A JGFormItemModel has no visual representation - it is strictly a logical entity, and the
  // member forms provide the user interface.  You can initialize a JGFormItemModel with a set of
  // member forms (by setting +link{JGFormItemModel.members} at init) or add and remove member
  // forms dynamically.
  // <P>
  // Calling +link{JGFormItemModel.setValues()} on a JGFormItemModel will automatically route new
  // field values to whichever member form is showing an editor for that field.  Likewise,
  // calling +link{JGFormItemModel.validate()} will validate all member forms, and
  // +link{JGFormItemModel.saveData()} will initiate a save operation which aggregates values from
  // all member forms.
  // <P>
  // Like a DynamicForm, a JGFormItemModel can be databound by setting
  // +link{JGFormItemModel.dataSource}.  In this case all member forms must also be bound to the
  // same DataSource.
  // <P>
  // In general, when working with a JGFormItemModel and its member forms, call APIs on the
  // JGFormItemModel whenever you are dealing with values that span multiple forms, and only call
  ...
  // JGFormItemModel may track values for which there is no FormItem in any member form.  However,
  // when using a JGFormItemModel these extra values are only allowed on the JGFormItemModel itself.
  ...
  //<
  isc.ClassFactory.defineClass('JGFormItemModel', 'ValuesManager')

  isc.JGFormItemModel.addClassMethods({
    //> @classMethod JGFormItemModel.getById()
    // Retrieve a JGFormItemModel by it's global +link{JGFormItemModel.ID,ID}.
    // @param ID (String) global ID of the JGFormItemModel
    // @return (JGFormItemModel) the JGFormItemModel, or null if not found
    // @visibility external
    //<
    //实例池
    _modelPool: {},
    ...
    getById: function (sId) {
      let vm = window[sId] || null
      return vm ? (isc.isA.JGFormItemModel(vm) ? vm : null) : null
    },

    getByDataSource: function (ds) {
      ds = isc.isA.String(ds) ? window[ds] : ds
      let id = ds.getID() + '_vm'
      let vm = isc.JGFormItemModel.getById(id)
      if (!vm) {
        vm = isc.JGFormItemModel.create({ ID: id, dataSource: ds })
        isc.JGFormItemModel.put(ds.getScopeId(), vm)
      }
      return vm
    },

    put: function (scopeId, model) {
      let pool = isc.JGFormItemModel._modelPool[scopeId]
      if (!pool) {
        pool = []
        isc.JGFormItemModel._modelPool[scopeId] = pool
      }
      pool.push(model)
    },
    ...
    destroy: function (scopeId) {
      let pool = isc.JGFormItemModel._modelPool[scopeId]
      if (pool) {
        delete isc.JGFormItemModel._modelPool[scopeId]
  ...
  isc.JGFormItemModel.addProperties({
    _bindFieldNames: null,

    disableValidation: true,

    //> @attr JGFormItemModel.dataSource  (DataSource | String : null : [IRWA])
    // Specifies a dataSource for this JGFormItemModel.  This dataSource will then be used for
    ...
    // @visibility external
    // @see JGFormItemModel.setDataSource()
    // @see JGFormItemModel.getDataSource()
    //<
    //dataSource: null,

    //> @attr JGFormItemModel.addOperation
    // +link{DSRequest.operationId,operationId} to use when performing add operations.
    // @include DataBoundComponent.addOperation
    // @visibility external
    //<

    //> @attr JGFormItemModel.updateOperation
    ...
    //> @attr JGFormItemModel.members (Array of DynamicForm : null : [IRW])
    // The set of member components for this JGFormItemModel.  These can be specified at init time
    // via the <code>members</code> property, or updated at runtime via <code>addMember()</code>
    // and <code>removeMember()</code>.<br>
    // Note: Alternatively a DataBoundComponent can be initialized as a member of a JGFormItemModel
    // by setting the <code>JGFormItemModel</code> property of the component to a pre-defined
    // JGFormItemModel instance, or by calling <code>setJGFormItemModel</code> on the component.
    // @visibility external
    // @see JGFormItemModel.addMember()
    // @see JGFormItemModel.removeMember()
    // @see dataBoundComponent.setJGFormItemModel()
    //<
    //members : null,

    //>	@attr JGFormItemModel.unknownErrorMessage	(string : null : [IRW])
    ...
    // +link{dataBoundComponent.unknownErrorMessage} when the JGFormItemModel is initialized.
    //<
    unknownErrorMessage: null

    //> @attr JGFormItemModel.disableValidation   (boolean : null : [IRWA])
    // @include DynamicForm.disableValidation
    //<

    //> @attr JGFormItemModel.autoSynchronize   (boolean : null : [IRWA])
    // If explicitly set to false, prevents the JGFormItemModel from automatically propagating
  ...
  //!>Deferred
  isc.JGFormItemModel.addMethods({
    ...
    init: function () {
      ...
      // inited _bindFieldNames property add by xiedh 2014-05-08
      this._bindFieldNames = []
      // end xiedh 2014-05-08
      ...
      isc.JGFormItemView._duplicateValues(this, this.values, {})
    ...
    // structure), whereas JGFormItemModels have to cope with any kind of data structure
    _saveDataReply: function (request, response, data) {
      if (
        !this.suppressServerDataSync &&
        response &&
        response.status >= 0 &&
        data != null
      ) {
        ...
        let memberForms = this.members ? this.members.duplicate() : []
        for (let i = 0; i < memberForms.length; i++) {
          let form = memberForms[i]
          if (!isc.isA.JGFormItemView(form)) continue
    ...
    _updateValue: function (fieldName, value, member) {
      //add by xiedh 2014-03-01
      if (this.values && this.values[fieldName] === value) return

      if (this._synchronizingMembers) return

      let isIndex = isc.isA.Number(fieldName)

      // warn on value with no associated items in dynamicForms
      //comment by xiedh 2014-02-27 对多值类控件数据同步有影响
      /*if (!isIndex && isc.isA.JGFormItemView(member) && member.getItem(fieldName) == null) {
      ...
      let dataPath = member.getFullDataPath()
      let field
      ...
      if (!isIndex) {
        ...
        // We were passed a record index, not a field name
        if (!dataPath) {
          dataPath = fieldName
    ...
    //> @method JGFormItemModel.synchronizeMembers()
    //   Update all of this JGFormItemModel's members to reflect the current values held by the
    //   JGFormItemModel.  It is not normally necessary to manually synchronize members, but you
    ...
    _combineDataPaths: function (baseDP, fieldDP) {
      ...
      return isc.JGFormItemView._combineDataPaths(baseDP, fieldDP)
    },
    ...
    _itemlessValueWarning: function (member, fieldName) {
      this.logWarn(
        'Member Form: ' +
          member +
          " has explicitly specified value for field[s] '" +
          fieldName +
          "', but has" +
          ...
          'on the JGFormItemModel via JGFormItemModel.setValues(), but not on ' +
          'member forms. See JGFormItemModel documentation for more info.'
      )
    },

    _clearValue: function (field, form) {
      let dataPath = form.getFullDataPath()
      if (dataPath) field = this._combineDataPaths(dataPath, field)
      return isc.JGFormItemView._clearFieldValue(field, this.values)
    },
    ...
    //> @method JGFormItemModel.bindToDataSource() ([A])
    //   Associate this JGFormItemModel with a DataSource.  Allows the developer to inherit
    //   properties from the DataSource fields to be applied to these values, such as validators,
    //   and to call data flow methods using this JGFormItemModel's data.
    ...
    //>@method  JGFormItemModel.setDataSource() (A)
    // Specifies a dataSource for this JGFormItemModel.  This dataSource will then be used for
    ...
    //>@method  JGFormItemModel.getDataSource() (A)
    // Returns the dataSource for this JGFormItemModel.  Will return null if this is not a
    // data-bound JGFormItemModel instance.
    // @visibility external
    // @return (DataSource)  Datasource object for this JGFormItemModel.
    //<
    ...
    lookupSchema: function () {
      ...
      let service
      if (this.serviceName)
        service = isc.WebService.getByName(
          this.serviceName,
      ...
      if ((this.serviceNamespace || this.serviceName) && service == null) {
        this.logWarn(
          'Could not find WebService definition: ' +
    ...
    //>@method JGFormItemModel.getItems()
    // Retrieves all form items contained within this JGFormItemModel's member forms
    // @return (array of FormItems) form items present in this JGFormItemModel
    //<
    getItems: function () {
      if (!this.members) return
      let items = []
      for (let i = 0; i < this.members.length; i++) {
        let form = this.members[i]
    ...
    //>@method JGFormItemModel.getItem()
    // Retrieve a +link{FormItem} from this JGFormItemModel.
    // <P>
    // Takes a field +link{formItem.name,name} or +link{type:DataPath}, and searches through the
    // members of this JGFormItemModel for an editor for that field. If found the appropriate
    ...
    //>@method  JGFormItemModel.getMembers()
    //  Retrieves an array of pointers to all the members for this JGFormItemModel.
    ...
    //>@method  JGFormItemModel.getMember()
    ...
    getMember: function (ID) {
      ...
      let member = window[ID]
      // sanity check
      if (this.members && this.members.contains(member)) return member
      return null
    },

    //>@method  JGFormItemModel.getMemberForField()
    ...
    _findMemberByField: function (fieldName, getItem, retrieveAll) {
      if (!this.members || fieldName == null || isc.isAn.emptyString(fieldName))
        return null
      ...
      for (let i = 0; i < this.members.length; i++) {
        ...
        if (member.getItem) {
          let field = this.members[i].getField(fieldName)
          /*if (!field) {
    ...
    getFileItemForm: function () {
      /* comment by xiedh 2014-02-10 没有用到文件控件
            if (!this.members) return;
            ...
            for (var i = 0; i < this.members.length; i++) {
                ...
                if (form) {
                    if (hasFileItemForm) {
                        this.logWarn("JGFormItemModel defined with more than one member form " +
                                " containing a FileItem. This is not supported - binary data may " +
                                "only be uploaded from one FileItem when saving JGFormItemModel data");                              
                    } else {
                        fileItemForm = form;
    ...
    //> @method JGFormItemModel.validate()
    // Validate the current set of values for this values manager against validators defined
    // in the member forms. For databound JGFormItemModels, also perform validation against any
    ...
    validate: function (
      ...
      // For databound JGFormItemModels, each member form will be responsible for validating
      ...
      // unless the data that came into the JGFormItemModel in the first place was in error)
      let returnVal = true,
      ...
      if (this.members) {
        ...
        for (let i = 0; i < this.members.length; i++) {
          if (!isc.isA.JGFormItemView(this.members[i])) continue
          let form = this.members[i],
          ...
          if (!disableValidation) {
            ...
            // form - instead we'll fire this method at the JGFormItemModel level only.
            ...
            if (form.handleHiddenValidationErrors != null) {
              this.logInfo(
                "form level 'handleHiddenValidationErrors' method suppressed " +
                  'in favor of JGFormItemModel level handler',
                'validation'
              )
          ...
          for (let j = 0; j < items.length; j++) {
            let fieldName =
              items[j].getFullDataPath() || items[j].getFieldName()
            ...
            if (
              dsFields &&
              this.members[i].getDataSource() == this.getDataSource()
            ) {
              delete dsFields[fieldName]
            }
            if (dataPaths && fieldName) {
              delete dataPaths[fieldName]
            }
          }
          ...
          let formSuccess = disableValidation
            ? true
      ...
      for (let fieldName in work) {
        let fieldObject = work[fieldName],
        ...
        if (validators != null) {
          ...
          for (let i = 0; i < validators.length; i++) {
            ...
            if (
              ...
              let errorMessage =
                validator.errorMessage || this.unknownErrorMessage
    ...
    buildDataPathsRecursively: function (dataPaths, parentDP, dataSource) {
      ...
      if (dataSource.__vmVisited) {
        this.logWarn(
          'detected ds loop at: ' + parentDP + ', refusing to recurse further'
        )
        return
      }
      ...
      for (let key in fields) {
        ...
        /*if (dataSource.fieldIsComplexType(key)) {//目前数据源字段都是简单类型 
    ...
    //> @method JGFormItemModel.getValidatedValues()
    // Call +link{JGFormItemModel.validate()} to check for validation errors. If no errors are found,
    // return the current values for this JGFormItemModel, otherwise return null.
    ...
    // addHiddenErrors()
    // For a JGFormItemModel, hidden validation errors may come from:
    ...
    addHiddenErrors: function (errors, form) {
      ...
      for (let fieldName in errors) {
        this.hiddenErrors[form][fieldName] = this._addFieldErrors(
    ...
    getHiddenErrors: function (suppressSynch) {
      ...
      let flatErrors = {}
      for (let form in this.hiddenErrors) {
        this.assembleHiddenErrorsRecursively(
          flatErrors,
          this.hiddenErrors[form]
        )
      }
      return flatErrors
    },

    assembleHiddenErrorsRecursively: function (
      flatErrors,
      formErrors,
      key,
      index
        ...
        for (let objKey in formErrors) {
          if (isc.isAn.Object(formErrors[objKey])) {
            ...
            if (outKey == '') {
              ...
              flatErrors[outKey + isc.Canvas._$slash + objKey] =
                formErrors[objKey]
            }
          }
        }
    ...
    synchHiddenErrors: function () {
      let hiddenErrors = this.hiddenErrors,
        vmID = this.getID()
      ...
      if (hiddenErrors && hiddenErrors[vmID]) {
        for (let field in hiddenErrors[vmID]) {
          let errors = hiddenErrors[vmID][field],
          ...
          if (item) {
            ...
            // clear out the hidden error under the JGFormItemModel's ID - the error
      ...
      let vmErrors = hiddenErrors[vmID]
      ...
      if (this.members) {
        for (let i = 0; i < this.members.length; i++) {
          ...
          if (!member.isVisible() || !member.isDrawn()) {
            ...
            for (let field in memberErrors) {
              let item = member.getItem(field)
              if (!item) {
    ...
    //>	@method	JGFormItemModel.processValidator()	(A)
    ...
    processValidator: function (item, validator, value, type, record) {
      return isc.Validator.processValidator(
        item,
        validator,
        value,
        type,
    ...
    _handleHiddenValidationErrors: function (suppressSynch) {
      let errors = this.getHiddenErrors(suppressSynch)
      ...
      let errorString = 'Validation failed with the following errors:'
      let errorArray = isc.isAn.Array(errors) ? errors : [errors]
      for (let i = 0; i < errorArray.length; i++) {
        let theErrors = errorArray[i]
        for (let fieldName in theErrors) {
    ...
    //>	@method	JGFormItemModel.setErrors() [A]
    // Sets validation errors for this JGFormItemModel to the specified set of errors.
    ...
    // +link{JGFormItemModel.handleHiddenValidationErrors()} will be fired instead.<br>
    // Note that if <code>showErrors</code> is false, errors may be shown at any time via
    // a call to +link{JGFormItemModel.showErrors()}.
    //
    ...
    setErrors: function (errors, showErrors) {
      ...
      //errors = isc.JGFormItemView.formatValidationErrors(errors);

      let memberForms = this.members ? this.members.duplicate() : []

      for (let i = 0; i < memberForms.length; i++) {
        if (!isc.isA.JGFormItemView(memberForms[i])) continue
        let form = memberForms[i],
        ...
        for (let j = 0; j < items.getLength(); j++) {
          let item = items[j],
            itemDataPath = item.getFullDataPath(),
            itemName = item.getFieldName(),
            itemErrors = this.getItemErrors(
              errors,
              itemDataPath,
              selectionChain
            )
    ...
    getItemErrors: function (errors, itemNameOrDataPath, selectionChain) {
      let dataPath = itemNameOrDataPath,
        isDataPath = false
      if (isc.isAn.Array(errors)) errors = errors[0]
      //if (!isDataPath) {
      let serverErrors = errors[itemNameOrDataPath]
      /*} else {
                var elements = dataPath.split(isc.Canvas._$slash),
                    serverErrors = errors,
                    nestedListCount = 0;
    ...
    //> @method JGFormItemModel.addFieldErrors()
    ...
    // firing +link{JGFormItemModel.handleHiddenValidationErrors()}.
    ...
    addFieldErrors: function (fieldName, errors, showErrors) {
      let hidden = true
      let form = this.getMemberForField(fieldName)
      if (form != null && isc.isA.JGFormItemView(form)) {
        form.addFieldErrors(fieldName, errors, false)
        let item = form.getItem()
      ...
      if (hidden) {
        if (!this.hiddenErrors) this.hiddenErrors = {}
        let formName = form ? form.getID() : this.getID()
        if (!this.hiddenErrors[formName]) this.hiddenErrors[formName] = {}

        this.hiddenErrors[formName][fieldName] = this._addFieldErrors(
    ...
    //> @method JGFormItemModel.setFieldErrors()
    // Sets validation errors for some field in the JGFormItemModel.<br>
    // Errors passed in should be a string (for a single error message) or an array of strings.
    // Pass in the showErrors parameter to immediately display the errors to the user by
    // redrawing the appropriate member form item (or if no visible item is found for the field
    // firing +link{JGFormItemModel.handleHiddenValidationErrors()}.
    ...
    //>	@method	JGFormItemModel.clearErrors()
    ...
    clearErrors: function (showErrors) {
      //comment by xiedh 2014-03-19 数据层无需清空绑定控件的异常提示
      //this.setErrors({}, showErrors);
    },

    //> @method JGFormItemModel.clearFieldErrors()
    ...
    clearFieldErrors: function (fieldName, show) {
      let form = this.getMemberForField(fieldName)
      if (form && isc.isA.JGFormItemView(form))
        form.clearFieldErrors(fieldName, show)

      if (this.hiddenErrors) delete this.hiddenErrors[fieldName]
    },

    //> @method JGFormItemModel.getErrors()
    // Returns the set of errors for this JGFormItemModel.
    ...
    //                  if there are no errors for this JGFormItemModel.
    // @group errors
    // @visibility external
    //<
    ...
    getErrors: function () {
      ...
      let errors = isc.addProperties({}, this.getHiddenErrors(true))
    ...
    //> @method JGFormItemModel.getFieldErrors()
    // Returns any validation errors for some field in this JGFormItemModel.
    ...
    //> @method JGFormItemModel.hasErrors()
    // Are there any errors associated with any fields in this JGFormItemModel?
    ...
    hasErrors: function () {
      if (this.hiddenErrors && !isc.isA.emptyObject(this.hiddenErrors)) {
        for (let form in this.hiddenErrors) {
          if (
            this.hiddenErrors[form] &&
            !isc.isAn.emptyObject(this.hiddenErrors[form])
          )
            return true
        }
      }
      ...
      for (let i = 0; i < this.members.length; i++) {
        if (
          isc.isA.JGFormItemView(this.members[i]) &&
          this.members[i].hasErrors()
        )
          return true
      }
      return false
    ...
    //> @method JGFormItemModel.hasFieldErrors()
    // Are there any errors associated with a field in this JGFormItemModel?
    ...
    hasFieldErrors: function (fieldName) {
      ...
      if (
        form &&
        isc.isA.JGFormItemView(form) &&
        form.hasFieldErrors(fieldName)
      )
        return true
      let hiddenErrors = this.getHiddenErrors(true)
      if (hiddenErrors && hiddenErrors[fieldName] != null) return true
      return false
    },

    //> @method JGFormItemModel.showErrors()
    ...
    // fire +link{JGFormItemModel.handleHiddenValidationErrors()} to allow custom handling of
    // hidden errors.
    // @group errors
    // @visibility external
    //<
    ...
    showErrors: function (suppressHiddenErrorSynch) {
      if (this.members) {
        for (let i = 0; i < this.members.length; i++) {
          if (!isc.isA.JGFormItemView(this.members[i])) continue
          if (!this.members[i].isDrawn() || !this.members[i].isVisible())
            continue
    ...
    // Otherwise +link{JGFormItemModel.handleHiddenValidationErrors()} will be fired to allow
    ...
    // Return the set of filter criteria for this JGFormItemModel based on its current set of
    // values
    ...
    getFilterCriteria: function () {
      ...
      isc.addProperties(
        crit,
        isc.DataSource.filterCriteriaForFormValues(values)
      )
    ...
    //> @method JGFormItemModel.getValues()
    ...
    // via +link{JGFormItemModel.setValues()}.
    ...
    getValues: function () {
      ...
      if (this.members != null) {
        let fc = isc.EH.getFocusCanvas()
        if (this.members.contains(fc) && fc.updateFocusItemValue)
          fc.updateFocusItemValue()
      }
    ...
    //> @method JGFormItemModel.setValues()
    // Replaces the current values of the JGFormItemModel and all member components with the
    // values passed in.
    // <P>
    ...
    // JGFormItemModel and returned by subsequent calls to +link{getValues()}.
    // <P>
    ...
    setValues: function (values) {
      if (isc.isAn.Array(values)) {
        let useFirst = isc.isA.Object(values[0])
        this.logWarn(
      ...
      //add by xiedh 2014-02-10
      if (
        this.values === values ||
        (isc.isAn.emptyObject(this._oldValues) && isc.isAn.emptyObject(values))
      ) {
        return
      }
      ...
      if (this._oldValues) {
        ...
        for (let field in values) {
          if (
            ...
            this._bindFieldNames.contains(field)
          ) {
            if (values[field] === this._oldValues[field]) {
              notChanged = true
            } else {
              notChanged = false
              break
      ...
      //end
      //comment by xiedh 2014-02-27
      /*var clonedVals = {};
            isc.JGFormItemView._duplicateValues(this, values, clonedVals);
            values = clonedVals;
            this.values = values;
            * */
      // add by xiedh 2014-02-27
      if (!isc.isA.emptyObject(values)) {
        let val = this.values || {}
        for (let attr in values) {
          if (
            values.hasOwnProperty(attr) &&
            this._bindFieldNames.contains(attr)
          ) {
            val[attr] = values[attr]
          }
        }
        this.values = val
      } else {
    ...
    //> @method JGFormItemModel.setData()
    // Set the data (values) on this JGFormItemModel [synonym for <code>setValues()</code>].
    //<
    ...
    //> @method JGFormItemModel.clearValues()
    ...
    clearValues: function () {
      this.setValues({})
      delete this.saveOperationType
    },

    //> @method JGFormItemModel.getMemberValues()
    // Returns the subset of this JGFormItemModel's values associated with some member form.
    //
    ...
    //> @method JGFormItemModel.setMemberValues()
    ...
    //> @method JGFormItemModel.rememberValues()
    ...
    //> @method JGFormItemModel.getOldValues()
    ...
    //> @method JGFormItemModel.getChangedValues()
    ...
    //> @method JGFormItemModel.resetValues()
    // @include dynamicForm.resetValues()
    //<
    resetValues: function () {
      // pull the values from form._oldValues into JGFormItemModel.values
      let values = {}
    ...
    //> @method JGFormItemModel.getValue()
    ...
    //                                  the potential records to use when the JGFormItemModel
    ...
    //> @method JGFormItemModel.setValue()
    ...
    setValue: function (fieldName, newValue) {
      let valueSet = false,
        ...
        undef
      if (this.members) {
        let items = this.getItem(fieldName, true)
      ...
      let members = this._findMemberByField(fieldName, false, true)
      if (members) {
        for (let i = 0; i < members.length; i++) {
          if (member && member.setData) {
            let dataObjPath = fieldName
            if (fieldName.indexOf(isc.Canvas._$slash) != -1) {
              dataObjPath = fieldName.substring(
                0,
                fieldName.lastIndexOf(isc.Canvas._$slash)
              )
              member.setData(
                isc.JGFormItemView._getFieldValue(
                  dataObjPath,
                  null,
                  this.values,
                  member,
    ...
    //> @method JGFormItemModel.clearValue()
    ...
    //> @method JGFormItemModel.addMember()
    //
    // Add a new member to this JGFormItemModel.  Any +link{class:Canvas} can be a member of a
    // JGFormItemModel, even components like +link{class:Layout} or +link{class:TabSet} that do
    // not actually have any values to manage.  When "valueless" components like these bind to
    // a JGFormItemModel, it is in order to provide their own child components with a shared
    // JGFormItemModel so that complex data can be displayed and edited - see
    // +link{DataBoundComponent.dataPath} for more details.
    // <p>
    ...
    // this JGFormItemModel.
    // <p>
    // Note on pre-existent values when the member component is a +link{class:DynamicForm}:<br>
    // If the JGFormItemModel has a value specified for some field, for which the member form has
    // an item, this value will be applied to the member form.  This is true whether the item
    // has a value or not.<br>
    // However if the member form has a value for some field, and the JGFormItemModel does not
    // have a specified value for the same field, we allow the JGFormItemModel to pick up the
    // value from the member form.
    //
    // @param   member  (DynamicForm | String) component (or ID of component) to add to
    //                                          this JGFormItemModel as a member.
    // @visibility external
    // @group members
    // @see method:JGFormItemModel.addMembers
    //<
    addMember: function (member, fromDataPath) {
      ...
      if (!isc.isA.Canvas(member)) {
        this.logWarn(
          'addMember() passed invalid object: ' +
            this.echo(member) +
            ' - this should be a Canvas instance'
        )
        return
      }
      if (member.JGFormItemModel != null)
        member.JGFormItemModel.removeMember(member)
      ...
      let memberDS = member.getDataSource()
      if (
        memberDS != null &&
        !fromDataPath &&
        memberDS != this.getDataSource()
      ) {
        this.logWarn(
          'addMember(): mismatched DataSources; new member form ' +
            member +
            " has dataSource: '" +
            memberDS.ID +
            "', JGFormItemModel has DataSource " +
      ...
      if (
        ...
        this.logWarn(
          'addMember(): new member form ' +
            ...
            'this member form will be dropped when saving values from this JGFormItemModel to ' +
            'the server.'
        )
      }
      ...
      if (member.dataArity == 'single' && member.autoTrackSelection) {
        ...
        if (
          member.selectionComponent == null ||
          member._autoSelectionComponent
        ) {
          let dataPath = member.getFullDataPath(),
            field = dataPath ? this.getDataSourceField(dataPath) : null,
            newValues = isc.JGFormItemView._getFieldValue(
              dataPath,
              null,
              this.values,
              member,
          ...
          if (multiple) {
            let selectionComponents = this.getMemberForField(dataPath, true)
            if (selectionComponents && selectionComponents.length > 0) {
              for (let i = 0; i < selectionComponents.length; i++) {
                let selectionComponent = selectionComponents[i]
        ...
        if (singularComponents && singularComponents.length > 0) {
          for (let i = 0; i < singularComponents.length; i++) {
            if (
              singularComponents[i].dataArity == 'single' &&
      ...
      if (member.dataArity == 'multiple' && member.autoTrackSelection) {
        let dataPath = member.getFullDataPath(),
          isDataPath = false //dataPath && dataPath.contains(isc.Canvas._$slash);
        ...
        /*if (isDataPath) {
                    ...
                    for (var i = elements.length - 2; i >= 0; i--) {
                        ...
                        var field = this.getDataSourceField(dataPath),
                            newValues = isc.JGFormItemView._getFieldValue(dataPath, null, this.values, member, true),
                            multiple = isc.isAn.Array(newValues) || (field && field.multiple);
                        if (multiple) break;
                    }
                }*/
        ...
        if (dataPath && dataPath != '') {
          ...
          for (let i = 0; i < members.length; i++) {
            ...
            let otherDataPath = members[i].getFullDataPath()
            if (
              otherDataPath &&
              ...
              if (members[i].selectionComponent != null) {
                if (members[i]._autoSelectionComponent) {
                  let existingDataPath =
                    members[i].selectionComponent.getFullDataPath()
      ...
      if (
        member.dataSource == null &&
        this.dataSource != null &&
        member.getFields
      ) {
        let fields = isc.isA.JGFormItemView(member)
          ? member._itemsConfig
          : member.getFields()
        ...
        if (dataPath) {
          let dataSource = dataSource.getDataSourceForDataPath(dataPath, true)
        }
      ...
      // This allows us to respect explicitly specified JGFormItemModel if the dataPath changes
      // later, but recalculate derived ones.
      member._JGFormItemModelFromDataPath = fromDataPath

      // We have directly manipulated the values object, so we should re remember it.
      this.rememberValues()

      //add by xiedh 2014-03-18
      if (member.getBoundFieldNames) {
        let boundFields = member.getBoundFieldNames()
        for (let i = 0, len = boundFields.length; i < len; i++) {
          let fieldName = boundFields[i]
          if (!this._bindFieldNames.contains(fieldName)) {
            this._bindFieldNames.push(fieldName)
          }
        }
      }
      let dataSource = this.getDataSource()
      let primaryKey = dataSource.getPrimaryKeyFieldName()
      if (!this._bindFieldNames.contains(primaryKey)) {
        this._bindFieldNames.push(primaryKey)
      }
      //end
    },

    // _setMemberValues - updates the values of a member (form or other dataBoundComponent) based
    // on the JGFormItemModel values.
    // if 'pickupMemberValues' is passed - for cases where the member has existing values
    // (and the JGFormItemModel doesn't) we pick up the field values from the member.
    // [if there are values specified on the vm and the member, the vm values will replace the
    //  members' values]
    // Called
    // - when a member is first added to this JGFormItemModel
    // - from JGFormItemModel.setValues()
    _setMemberValues: function (member, pickupMemberValues) {
      //newValObj = isc.clone(this.values) , ������¼  {'tt2':value, 't1':value2,'fieldData1':'11'}
      //member.values, ��ǰ�?�ؼ��ļ�¼ {'tt2':value, 't1':value2}
      let newObj = {},
        itemFields = member.getBoundFieldNames(),
        data = member.getData(),
        notChanged = true /**member.values;
            if(member.getItemFields){
                itemFields = member.getItemFields();
            }*/

      /**
            for(var memKey in itemFields) {
                var recordValue = this.values[memKey];
                if(recordValue){
                    newObj[memKey] = recordValue;
                }    		
            }*/
      for (let i = 0; i < itemFields.length; i++) {
        let itemField = itemFields[i]
        let recordValue = this.values[itemField]
        if (!notChanged || !data || data[itemField] !== recordValue) {
          notChanged = false
        }
        newObj[itemField] = recordValue
      }
      if (notChanged) return
      if (this.values['id'] && !isc.isA.emptyObject(newObj)) {
        newObj['id'] = this.values['id']
      }

      member.setData(!isc.isA.emptyObject(newObj) ? newObj : this.values)
      //add by xiedh 2014-03-26 加入校验
      if (member.validate) {
        member.validate()
      }

      /** if (member.getItems == null) return;
            var items = member.getFieldItems(), //member.getItems(),
                hasChanges = false,
                len = items.getLength(),
                fieldUpdated = {},
                undef;
            for (var i = 0; i < len; i++) {
                var item = items[i];
                var itemPath = item.getFieldName();
                if (itemPath == null||fieldUpdated.hasOwnProperty(itemPath)) continue;
                var newFieldValue = isc.JGFormItemView._getFieldValue(itemPath, null,this.values,member, true);
                var currentFieldValue = isc.JGFormItemView._getFieldValue(itemPath, null,member.values,member, true);//item._value;//
                //���ֵ��ͬ���������                                           
                ...
                if (currentFieldValue !== undef) {
                     this.logInfo("Member form " + member +
                            " has specified value for field '" + itemPath +  
                            "' which collides with an already specified value in this " +
                            "JGFormItemModel. Resetting the value on the member form.");
                }
                ...
                if (item.shouldSaveValue != false) {
                    ...
                    if (updatedFieldVal === undef) {
                        isc.JGFormItemView._clearFieldValue(itemPath, this.values, member, true);
                     } else {
                         isc.JGFormItemView._saveFieldValue(itemPath, null,updatedFieldVal,this.values, member, true);
                     }
                     hasChanges = true;
                }
                ...
                fieldUpdated[itemPath] = true;
            }
            if (hasChanges) {
                if (member.valuesChanged) member.valuesChanged();
                if (member.markForRedraw) member.markForRedraw("setItemValue");
            }
    ...
    // When we first add a DynamicForm to a JGFormItemModel it may have values for fields
    ...
    _findItemlessFormValues: function (
      form,
      values,
      dataPath,
      itemlessValues,
      ...
      for (let prop in values) {
        let fieldID = dataPath ? this._combineDataPaths(dataPath, prop) : prop
        if (!form.getItem(fieldID)) {
          let value = values[prop]
          if (
            !isc.isAn.Object(value) ||
            isc.isA.Date(value) ||
            ...
            /*
                        var fullDataPath = form.dataPath 
                                                ...
                                                : fieldID,
                            currentValue = isc.JGFormItemView._getFieldValue(prop, null, this.values),
                            undef;
                        if (currentValue === undef) {
                            isc.JGFormItemView._saveFieldValue(fullDataPath, null value, this.values);
                        }
                        */
          } else {
    ...
    //> @method JGFormItemModel.addMembers()
    //  Add multiple new member forms to this JGFormItemModel.
    // @param   members  (Array of DynamicForm) array of forms to add to this JGFormItemModel as members.
    // @group members
    // @see method:JGFormItemModel.addMember
    // @visibility external
    //<
    ...
    //> @method JGFormItemModel.removeMember()
    //  Remove a member form from this JGFormItemModel, so its values are no longer managed
    //  by this instance.
    //  This does not clear the values associated with the form from the JGFormItemModel - they
    //  will still be available via <code>JGFormItemModel.getValues()</code>, but will not be
    //  updated as the form is manipulated.
    // @param   member  (DynamicForm | String)
    //      form (or ID of form) to remove from this JGFormItemModel
    // @group members
    // @see method:JGFormItemModel.removeMembers()
    // @visibility external
    //<
    ...
    //> @method JGFormItemModel.removeMembers()
    // Remove multiple member forms from this JGFormItemModel.
    // @param members (Array of DynamicForm) array of forms to remove
    // @group members
    // @see method:JGFormItemModel.removeMember()
    // @visibility external
    //<
    ...
    // Display
    // ----------------------------------------------------------------------------------------
    // JGFormItemModels don't usually display their values directly - but support
    ...
    getPrintHTML: function () {
      ...
      printHTML.append(
        "<TABLE border=1><TR><TD align='center' style='font-weight:bold;'>Field</TD>",
  ...
  isc.JGFormItemModel.registerStringMethods({
    //> @method JGFormItemModel.handleHiddenValidationErrors (A)
    // Method to display validation error messages for a JGFormItemModel when there is not
    ...
    // - for databound JGFormItemModels, a datasource field with specified validators, but not
    ...
    //>	@method JGFormItemModel.submitValues()
    // Optional +link{stringMethod} to fire when +link{JGFormItemModel.submit()} is called
    // on this JGFormItemModel (or any form included in this JGFormItemModel).
    //
    // @param	values    (object)        the JGFormItemModel values
    // @param	JGFormItemModel      (JGFormItemModel)   the JGFormItemModel being submitted
    // @group submitting
    // @see method:JGFormItemModel.submit()
    // @visibility external
    //<
    submitValues: 'values,JGFormItemModel'
  })
  //!<Deferred
}
