...
function charToHex(ch) {
  ...
  if (ch <= 255 && ch >= 0) {
    return hexes[ch >> 4] + hexes[ch & 0x0f]
  } else {
    return charToHex((ch & 0xff00) >> 8) + charToHex(ch & 0x00ff)
  }
}
...
function isHexString(s) {
  ...
  for (let i = 0; i < s.length; i++) {
    let ch = i == s.length - 1 ? s.substring(i) : s.substring(i, i + 1)
    if (hexstr.indexOf(ch) < 0) return false
  }
  return true
}
...
function hexToString(s) {
  ...
  for (let i = 0; i < s.length; i += 2) {
    let sxx = parseInt(s.substring(i, i + 2), 16)
