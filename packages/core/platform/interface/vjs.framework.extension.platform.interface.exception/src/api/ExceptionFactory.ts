...
let getCausedby = function (params: any) {
  return {
...
const isException = function (e: any) {
...
const isAcceptType = function (type: any) {
  if (
    type &&
    (type == TYPES.Business ||
      type == TYPES.Unlogin ||
...
const genError = function (msg: string, type: any) {
...
const getExceptionTypeByError = function (error: any, defaultType: string) {
...
const unSerialize = function (json: any) {
  if (!json) {
    return null
  }
  ...
  let type = json.name
  let error = new Error(message, type)
  let constructor = constructors[type] || constructors['SystemException']
