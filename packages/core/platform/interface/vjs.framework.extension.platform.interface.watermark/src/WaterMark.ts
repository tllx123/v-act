...
const draw = function (params: Record<string, any>) {
  ...
  let windowContainerId: string =
    windowContainerManager.getByScopeId(scopeId) || '#############'
  ...
  if (windowContainer) {
    ...
    if (dom) {
      ...
      canvas.height = calHeight(content, canvas, 58)
      ...
      imgObj.onload = scopeManager.createScopeHandler({
        ...
        handler: function () {
          ...
          writerContent(content, canvas, 0, 58)
