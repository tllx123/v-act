...
export function copyByEntity(params, routeContext) {
  ...
  if (operType == OPERATOR_TYPE.APPEND) {
    changedRecords = _append2Dest(dest, sourceMappingRecords, fieldMapping)
  } else {
...
const _getEntityRecords = function (
  ...
  switch (copyType) {
    ...
    case COPY_TYPE.SELECTEDROWS_CURRENTROW:
      ...
      const currentSeled = datasource.getCurrentRecord() //当前选中数据（高亮）
      if (currentSeled) {
        ...
        for (let i = 0; i < records.length; i++) {
          ...
          if (record.getSysId() == currentSeled.getSysId()) {
            exist = true
            break
          }
        }
        if (!exist) records.push(currentSeled)
      }
      break
