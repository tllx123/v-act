...
;(function (root, factory) {
  ...
  'use strict'
  let PhotoSwipe = function (template, UiClass, items, options) {
    /*>>framework-bridge*/
    /**
     *
    ...
    if (framework.features.oldIE) {
      framework.bind = function (target, type, listener, unbind) {
        ...
        let methodName = (unbind ? 'detach' : 'attach') + 'Event',
          evName,
          _handleEv = function () {
    ...
    let _isOpen,
      ...
      _calculatePanOffset = function (axis, zoomLevel) {
        let m = _midZoomPoint[axis] - _offset[axis]
        return (
      ...
      _calculatePanBounds = function (zoomLevel, update) {
        let bounds = _calculateItemSize(self.currItem, _viewportSize, zoomLevel)
        if (update) {
      ...
      _setupTransforms = function () {
        if (_transformKey) {
          // setup 3d transforms
          let allow3dTransform = _features.perspective && !_likelyTouchDevice
        ...
        _applyZoomPanToItem = function (item) {
          let zoomRatio = item.fitRatio > 1 ? 1 : item.fitRatio,
    ...
    let _animations = {},
      ...
      _animateProp = function (
        ...
        let startAnimTime = _getCurrentTime(),
          t
    ...
    let _gestureStartTime,
      ...
      _pushPosPoint = function (time, x, y) {
        if (time - _gestureCheckSpeedTime > 50) {
          let o = _posPoints.length > 2 ? _posPoints.shift() : {}
          o.x = x
          o.y = y
          _posPoints.push(o)
      ...
      _calculateVerticalDragOpacityRatio = function () {
        let yOffset = _panOffset.y - self.currItem.initialPosition.y // difference between initial and current position
      ...
      _onDragStart = function (e) {
        ...
        if (_pointerEventEnabled) {
          let pointerIndex = framework.arraySearch(
            _currPointers,
            e.pointerId,
            'id'
          )
        ...
        let startPointsList = _getTouchPoints(e),
      ...
      _onDragMove = function (e) {
        ...
        if (_pointerEventEnabled) {
          ...
          if (pointerIndex > -1) {
            let p = _currPointers[pointerIndex]
            p.x = e.pageX
            p.y = e.pageY
          }
        }
        ...
        if (_isDragging) {
          let touchesList = _getTouchPoints(e)
      ...
      _renderMovement = function () {
        ...
        let numPoints = _currentPoints.length
        ...
        if (_isZooming && numPoints > 1) {
          ...
          let pointsDistance = _calculatePointsDistance(p, p2)
          ...
          if (zoomLevel < minZoomLevel) {
            if (
              ...
              let minusDiff = minZoomLevel - zoomLevel
              let percent = 1 - minusDiff / (minZoomLevel / 1.2)
          ...
          if (_direction === 'v' && _options.closeOnVerticalDrag) {
            if (!_canPan()) {
              ...
              let opacityRatio = _calculateVerticalDragOpacityRatio()
          ...
          let mainScrollChanged = _panOrMoveMainScroll('x', delta)
      ...
      _onDragRelease = function (e) {
        ...
        let touchList = _getTouchPoints(e),
          gestureType,
        ...
        if ((_mainScrollShifted || _mainScrollAnimating) && numPoints === 0) {
          let itemChanged = _finishSwipeMainScrollGesture(
            gestureType,
            _releaseAnimData
          )
          if (itemChanged) {
      ...
      _initDragReleaseAnimationData = function () {
        // temp local vars
        let lastFlickDuration, tempReleasePos
      ...
      _finishSwipeMainScrollGesture = function (gestureType, _releaseAnimData) {
        ...
        if (gestureType === 'swipe') {
          let totalShiftDist = _currPoint.x - _startPoint.x,
        ...
        let animateToX = _slideSize.x * _currPositionIndex
        let animateToDist = Math.abs(animateToX - _mainScrollPos.x)
    ...
    let _showOrHideTimeout,
      _showOrHide = function (item, img, out, completeFn) {
        ...
        let startAnimation = function () {
          let closeWithRaf = _closedByScroll,
            fadeEverything =
              !self.currItem.src ||
    ...
    let _getItemAt,
      ...
      _calculateItemSize = function (item, viewportSize, zoomLevel) {
        if (item.src && !item.loadError) {
          ...
          if (isInitial) {
            let hRatio = _tempPanAreaSize.x / item.w
            let vRatio = _tempPanAreaSize.y / item.h

            item.fitRatio = hRatio < vRatio ? hRatio : vRatio
            //item.fillRatio = hRatio > vRatio ? hRatio : vRatio;

            let scaleMode = _options.scaleMode
      ...
      _preloadImage = function (item) {
        item.loading = true
        item.loaded = false
        let img = (item.img = framework.createEl('pswp__img', 'img'))
        let onComplete = function () {
          item.loading = false
          item.loaded = true
      ...
      _setImageSize = function (item, img, maxRes) {
        ...
        let w = maxRes ? item.w : Math.round(item.w * item.fitRatio),
    ...
    let _historyUpdateTimeout,
      ...
      _updateHash = function () {
        ...
        let item = _getItemAt(_currentItemIndex)
        ...
        let newURL = _windowLoc.href.split('#')[0] + '#' + newHash
