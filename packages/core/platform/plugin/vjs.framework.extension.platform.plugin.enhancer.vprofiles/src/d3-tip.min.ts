!(function (a, b) {
  ...
  return function () {
    ...
    function o() {
      let a = z()
      return {
        top: a.n.y - e.offsetHeight,
        left: event.clientX - e.offsetWidth / 2
      }
    }
    function p() {
      let a = z()
    ...
    function w() {
      let b = a.select(document.createElement('div'))
      return (
        b.style({
          'position': 'absolute',
          'top': 0,
    ...
    let m = a.map({ n: o, s: p, e: q, w: r, nw: s, ne: t, sw: u, se: v }),
      n = m.keys()
    return i
  }
})
