      ...
      if (parentWidgetCode) {
        ...
        switch (parentProperty._$WidgetType || parentProperty.WidgetType) {
          ...
          case 'JGTreeGrid':
            ...
            if ((!fields || fields.length < 1) && (!tools || tools.length < 1))
              continue
...
var setToolbarsProp = function (toolbars, modifyProperty, wId, childPropertys) {
  if (toolbars && toolbars.length > 0) {
    for (var i = 0; i < toolbars.length; i++) {
      ...
      setToolbarsProp(toolbars[i].toolbars, modifyProperty, wId, childPropertys)
    }
  }
  return childPropertys
}
