exports.initModule = function (sBox) {
  ...
  isc.ValuesManager.addClassMethods({
    ...
    // Retrieve a ValuesManager by it's global +link{valuesManager.ID,ID}.
  ...
  isc.ValuesManager.addProperties({
    ...
    // @see dataBoundComponent.setValuesManager()
    //<
    //members : null,
  ...
  isc.ValuesManager.addMethods({
    ...
    _updateValue: function (fieldName, value, member) {
      ...
      isc.DynamicForm._saveFieldValue(
        dataPath,
        field,
        value,
        this.values,
      ...
      let isDataPath = dataPath.contains(isc.Canvas._$slash)
      if (isDataPath && this.autoSynchronize !== false) {
        let elements = dataPath.split(isc.Canvas._$slash)
        if (
          parseInt(elements[elements.length - 1]) ==
          ...
          let fields = this.getFieldsForDataPath(dataPath || fieldName)
    ...
    getDataSourceField: function (fieldID) {
      ...
      for (let i = 0; i < segments.length; i++) {
        ...
        let fieldId = segments[i]
        field = dataSource.getField(fieldId)
        dataSource = field
          ? isc.DataSource.getDataSource(field.type)
          : dataSource
      }
      return field
    },
    ...
    _findMemberByField: function (fieldName, getItem, retrieveAll) {
      ...
      let trimmedFieldName = fieldName.trim(isc.Canvas._$slash)

      // determine whether the fieldName passed in was a dataPath once before
      // looping through members
      let dataPathSegments = trimmedFieldName.split(isc.Canvas._$slash)
      ...
      for (let i = 0; i < this.members.length; i++) {
        ...
        if (
          memberDataPath == isc.Canvas._$slash ||
          isc.isAn.emptyString(memberDataPath)
        ) {
          memberDataPath = null
        ...
        if (
          ...
          let soFar = null
          for (let ii = 0; ii < dataPathSegments.length; ii++) {
            ...
            if (memberDataPath.endsWith(isc.Canvas._$slash)) {
              memberDataPath = memberDataPath.substring(
                0,
                memberDataPath.length - 1
              )
            }
    ...
    getFileItemForm: function () {
      if (!this.members) return
      let hasFileItemForm = false,
        fileItemForm
      for (let i = 0; i < this.members.length; i++) {
        if (this.members[i].getFileItemForm == null) continue
        let form = this.members[i].getFileItemForm()
        if (form) {
          if (hasFileItemForm) {
            this.logWarn(
              'ValuesManager defined with more than one member form ' +
    ...
    validate: function (
      ...
      if (this.members) {
        ...
        for (let i = 0; i < this.members.length; i++) {
            ...
            if (form.handleHiddenValidationErrors != null) {
              this.logInfo(
                "form level 'handleHiddenValidationErrors' method suppressed " +
                  'in favor of valuesManager level handler',
                'validation'
              )
          ...
          for (let j = 0; j < items.length; j++) {
            ...
            isDataPath =
              isDataPath ||
              (fieldName && fieldName.contains(isc.Canvas._$slash))
      ...
      for (let fieldName in work) {
        let fieldObject = work[fieldName],
          ...
          value = isc.DynamicForm._getFieldValue(
            fieldName,
            fieldObject,
            values,
            null,
    ...
    buildDataPathsRecursively: function (dataPaths, parentDP, dataSource) {
      ...
      for (let key in fields) {
        ...
        if (dataSource.fieldIsComplexType(key)) {
          let subDS = dataSource.getSchema(fields[key].type)
          this.buildDataPathsRecursively(
            dataPaths,
            parentDP + key + isc.Canvas._$slash,
            subDS
          )
        }
      }
    ...
    getItemErrors: function (errors, itemNameOrDataPath, selectionChain) {
      let dataPath = itemNameOrDataPath.trim(isc.Canvas._$slash),
    ...
    deleteItemErrors: function (errors, itemNameOrDataPath, selectionChain) {
      ...
      if (!isDataPath) {
        ...
        for (let i = traversedObjects.length - 1; i >= 0; i--) {
    ...
    clearFieldErrors: function (fieldName, show) {
      let form = this.getMemberForField(fieldName)
      if (form && isc.isA.DynamicForm(form))
    ...
    hasErrors: function () {
      ...
      for (let i = 0; i < this.members.length; i++) {
        if (isc.isA.DynamicForm(this.members[i]) && this.members[i].hasErrors())
          return true
      }
      return false
    },
    ...
    hasFieldErrors: function (fieldName) {
      let form = this.getMemberForField(fieldName)
      if (form && isc.isA.DynamicForm(form) && form.hasFieldErrors(fieldName))
        return true
    ...
    resetValues: function () {
      ...
      for (let i = 0; i < this._rememberedDefaults; i++) {
        isc.DynamicForm._clearFieldValue(
    ...
    valuesHaveChanged: function (returnChangedVals) {
      ...
      return isc.DynamicForm.valuesHaveChanged(
        this,
        returnChangedVals,
        values,
        oldValues
    ...
    getValue: function (fieldName, component) {
      return isc.DynamicForm._getFieldValue(
        fieldName,
        ...
        for (let i = 0; i < members.length; i++) {
          if (member && member.setData) {
            ...
            if (fieldName.indexOf(isc.Canvas._$slash) != -1) {
              ...
              member.setData(
                isc.DynamicForm._getFieldValue(
                  dataObjPath,
                  null,
                  this.values,
                  member,
    ...
    addMember: function (member, fromDataPath) {
      ...
      if (member.dataArity == 'single' && member.autoTrackSelection) {
        ...
        if (
          ...
          let dataPath = member.getFullDataPath(),
            ...
            newValues = isc.DynamicForm._getFieldValue(
              dataPath,
              null,
              this.values,
              member,
      ...
      if (member.dataArity == 'multiple' && member.autoTrackSelection) {
        ...
        if (isDataPath) {
          ...
          for (let i = elements.length - 2; i >= 0; i--) {
            ...
            let field = this.getDataSourceField(dataPath),
              newValues = isc.DynamicForm._getFieldValue(
                dataPath,
                null,
                this.values,
      ...
      if (
        ...
        let fields = isc.isA.DynamicForm(member)
          ? member._itemsConfig
          : member.getFields()
    ...
    _setMemberValues: function (member, pickupMemberValues) {
      ...
      let memberDataPath = member.getFullDataPath(),
      ...
      if (multiple) {
        if (member.dataArity == 'single') {
          ...
          if (selComponent != null) {
            ...
            let record = member._selectionComponentRecordPKs,
              foundRecord = false
            ...
            if (foundRecord) {
              ...
              this.logDebug(
                "Unable to locate selectionComponent's selected record in " +
                  '_setMemberValues.  Record is: ' +
                  isc.Comm.serialize(record)
              )
            }
          } else {
        ...
        if (newValues != null && member.dataArity == 'multiple')
          newValues = [newValues]
      }
      ...
      if (!isc.isA.DynamicForm(member)) {
        ...
        if (newValues == null) {
          ...
          if (
            pickupMemberValues &&
            oldValues != null &&
            !isc.isAn.emptyObject(oldValues) &&
            !isc.isAn.emptyArray(oldValues)
          ) {
            this.logInfo(
              'ValuesManager member:' +
                member.getID() +
                ' has existing values:' +
                this.echo(oldValues) +
                ', replacing with values from this valuesManager:' +
                this.echo(newValues)
            )
          }
        ...
        for (let i = 0; i < len; i++) {
          ...
          if (!isAbsolute) {
            let currentFieldValue = isc.DynamicForm._getFieldValue(
              fullFieldPath,
              null,
              member.values,
              member,
          ...
          if (item.shouldSaveValue != false) {
            let updatedFieldVal = member.getValue(itemPath)
          ...
          if (
            ...
            isc.isA.Function(item.formValuesChanged)
          )
            item.formValuesChanged()
        }
