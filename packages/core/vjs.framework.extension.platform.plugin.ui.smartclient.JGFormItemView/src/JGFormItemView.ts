exports.initModule = function (sBox) {
   ...
   * Version v9.0p_2014-01-05 (2014-01-05)
  ...
  //>	@class	JGFormItemView
  //
  // The JGFormItemView manages a collection of FormItems which represent user input controls.  The
  // JGFormItemView provides +link{group:FormLayout,layout}, value management, validation and
  // databinding for the controls it manages.
  // <P>
  // <var class="smartgwt">
  // To create a JGFormItemView, create several +link{FormItem}s and pass them to
  // +link{setItems()}.  For example:
  // <pre>
  //		    JGFormItemView form = new JGFormItemView();
  ...
  // To create a JGFormItemView, set +link{dynamicForm.fields} to an Array of Objects describing the
  // FormItems you want to use.  For example:
  // <pre>
  //		    isc.JGFormItemView.create({
  //		        fields:[
  ...
  // FormItems.
  // <P>
  // NOTE: For very simple forms consisting of exactly one item, you still use a JGFormItemView.
  ...
  isc.ClassFactory.defineClass('JGFormItemView', 'JGBaseFormWidget')

  // Synonym for use by ValuesManagers working with distributed 'FormLayouts'
  isc.addGlobal('FormLayout', isc.JGFormItemView)
  ...
  // add constants
  isc.JGFormItemView.addClassProperties({
    //>	@type	FormMethod
    //			Form METHOD parameters - how the form fields are submitted to the server
    GET: 'GET', //	@value	isc.JGFormItemView.GET		GET request -- URL encoding (~4K max)
    POST: 'POST', //	@value	isc.JGFormItemView.POST	POST request -- separate field encoding (no max)
    //			@visibility external
    //			@group	submitting
    //<
    ...
    // @value JGFormItemView.NORMAL    normal form encoding ("application/x-www-form-urlencoded")
    NORMAL: 'normal',
    // @value JGFormItemView.MULTIPART form encoding for forms with INPUT file elements, that is, forms
    ...
    buildOperatorIndex: function () {
      ...
      let list = isc.getValues(isc.DataSource.getSearchOperators())

      list = list.sortByProperties(
        ['symbol'],
        [false],
        [
    ...
    _defaultItemHoverHTMLImpl: function (item) {
      ...
      if (!prompt && item.parentItem)
        prompt = this.itemHoverHTML(item.parentItem)
      return prompt
    },

    _defaultValueHoverHTMLImpl: function (item) {
      let returnVal = item.getDisplayValue()
      if (returnVal != null) {
        returnVal = '' + returnVal
  ...
  isc.JGFormItemView.addProperties({
    ...
    // <P>
    // +link{JGFormItemView.colWidths} controls the widths of form columns.  FormItems that have
    ...
    // column to overflow as described under +link{JGFormItemView.fixedColWidths}.
    // <P>
    ...
    // <P>
    // Individual item heights are controlled by +link{item.height}. This may be specified as
    ...
    // rendered.<br>
    // Note that +link{item.cellHeight} may be specified to explicitly control the height of
    // an item's cell. In this case the specified +link{item.height} will govern the size
    ...
    //> @attr JGFormItemView.sectionVisibilityMode (VisibilityMode : "multiple" : [IRW])
    ...
    // JGFormItemView level as a default, or on individual items.
    //
    // @value  "left"
    // @value  "top"
    // @value  "right"
    // @group formTitles
    // @see JGFormItemView.titleOrientation
    ...
    // are all boolean properties, and can be set on a JGFormItemView to control the behavior
    ...
    // (see +link{JGFormItemView.focusInItem()}), all text within the item will be selected.
    // <P>
    ...
    // programmatically (see +link{JGFormItemView.focusInItem()}), or via a mouse click, all text
    ...
    // Spellcheck:
    //>@attr    JGFormItemView.browserSpellCheck   (Boolean : true : IRW)
    ...
    // <P>
    // If set, calling +link{method:JGFormItemView.submit()} causes an RPC to be sent to this URL to
    ...
    // succeeds, the form is submitted to the URL specified by +link{attr:JGFormItemView.action}.
    // <p>
    ...
    // @group validation
    // @visibility external
    // @see JGFormItemView.saveData()
    // @see JGFormItemView.submit()
    //<
    ...
    // The name of the special field sent to the server as part of +link{method:JGFormItemView.cancel()}
    // @visibility external
    //<
    cancelParamName: 'org.apache.struts.taglib.html.CANCEL',

    //> @attr dynamicForm.cancelParamValue (String : "cancel" : IRW)
    // The value of the special field sent to the server as part of +link{method:JGFormItemView.cancel()}
    // @visibility external
    //<
    ...
    // See +link{JGFormItemView.canSubmit} for more on this.
    //
    ...
    //>	@attr	dynamicForm.method		(FormMethod : isc.JGFormItemView.POST : [IRW])
    ...
    //      @visibility external
    //<
    method: isc.JGFormItemView.POST,

    //>	@attr	dynamicForm.encoding		(Encoding : JGFormItemView.NORMAL : IRWA)
    // encoding for the form, use MULTIPART_ENCODING for file upload forms
    // @group submitting
    // @visibility external
    //<
    encoding: isc.JGFormItemView.NORMAL_ENCODING,
    ...
    // Note that if true, +link{JGFormItemView.submit()} will perform a native HTML submission
    // to the specified +link{JGFormItemView.action} URL.<br>
    ...
    // If set, this <code>JGFormItemView</code> will set both
    ...
    // +link{class:FormItem} class to use for any list-type complex fields on this JGFormItemView.
  ...
  // add default methods
  isc.JGFormItemView.addMethods({
    //获取重画原因 add by xiedh 2014-02-13
    getRedrawReason: function () {
      return this._redrawReason
    },
    //设置重画原因 add by xiedh 2014-02-13
    setRedrawReason: function (reason) {
      this._redrawReason = reason
    },
    //清除重画原因 add by xiedh 2014-02-13
    clearRedrawReason: function () {
      this._redrawReason = null
    },
    // edit by xiedh 2011-02-19
    markForRedraw: function (reason) {
      this.setRedrawReason(reason)
      if (reason != 'bind') {
        this.Super('markForRedraw', arguments)
      }
    },
    // add by xiedh 2014-03-05
    couldRedraw: function () {
      return true
    },
    /*
        // add by xiedh 2011-02-19
            ...
            /*if (this.parentElement) {
                if (this._drewClipDiv) {
                    //this.logWarn("caching handle: " + this.echoLeaf(newElement));
                        this._handle = this.parentElement.getHandle();
                        this._clipDiv = this._handle.parentElement;
                    } else {
                        this._handle = this.parentElement.getHandle();
                    }
                }
    ...
    initWidget: function () {
      ...
      if (!isc.JGFormItemView._operatorIndex)
        isc.JGFormItemView.buildOperatorIndex()
      ...
      // When creating a JGFormItemView subclass, for which each instance should show a specific set
      ...
      // flag
      if (
        this.action != isc.JGFormItemView.getPrototype().action &&
        this.action != null &&
        !isc.isA.emptyString(this.action)
      ) {
      ...
      if (this.valuesManager != null) {
        ...
        if (isc.isA.ValuesManager(this.valuesManager)) {
          if (
            ...
            this.valuesManager.dataSource != null
          ) {
      ...
      if (this.selectionComponent != null)
        this.setSelectionComponent(this.selectionComponent, true)
    },
    ...
    setHandleDisabled: function (disabled) {
      ...
      for (let i = 0; i < items.length; i++) {
        items[i].updateDisabled(disabled)
      }
    },
    ...
    // Synonym for +link{JGFormItemView.setFields()}
    //
    // @group elements
    ...
    setItems: function (itemList, firstInit) {
      ...
      if (itemList != null) {
        for (let i = 0; i < itemList.length; i++) {
          ...
          if (itemList[i] == null) {
            this.logWarn(
              'Encountered empty entry in items array - removing this entry.'
            )
            invalidItem = true
          }
          if (isc.isA.Canvas(itemList[i])) {
            this.logWarn(
              'Encountered a Canvas instance:' +
                itemList[i] +
                ' in the items ' +
                'array - the JGFormItemView items array should contain only FormItem ' +
    ...
    // Set the +link{dynamicForm.fields,items} for this JGFormItemView.  Takes an array of item
    ...
    // @visibility external
    //<
    // edit by xiedh 2014-02-19
    setFields: function (fieldList) {
      if (this.isDrawn() && this.items == fieldList) {
        return
      }
    ...
    // Method to retrieve the +link{dynamicForm.fields, items} for this JGFormItemView.
    //
    ...
    visibleAtPoint: function (x, y, withinViewport, ignoreWidgets) {
      ...
      for (let i = -1; i < items.length; i++) {
        ...
        if (containerWidgets[cwID] == null) {
          containerWidgets[cwID] = cw.visibleAtPoint(
            x,
            y,
            withinViewport,
            ignoreWidgets
        ...
        let PL = item.getPageLeft(),
          PT = item.getPageTop()
        if (
          PL <= x &&
          PL + item.getVisibleWidth() >= x &&
          PT <= y &&
          PT + item.getVisibleHeight() >= y
        ) {
          return true
        }
      }
    ...
    _$upload: 'upload',
    _$uploadItem: 'UploadItem',
    _$tUploadItem: 'TUploadItem',
    _$mutex: 'mutex',
    _addItems: function (newItems, position, fromSetItems, firstInit) {
      ...
      let haveUploadFields = false,
        foundFileItem = false,
        mutexSections = this.sectionVisibilityMode == this._$mutex

      for (let itemNum = 0; itemNum < newItems.length; itemNum++) {
        let item = newItems[itemNum]
        ...
        let canEditItem = isc.JGFormItemView.canEditField(item, this)
        ...
        let itemType = this.getEditorType(item)
        newItems[itemNum] = item = this.createItem(item, itemType)

        if (
          itemType == this._$upload ||
          itemType == this._$uploadItem ||
        ...
        if (isc.FileItem && isc.isA.FileItem(item) && foundFileItem) {
          this.logWarn(
            'Attempting to creating a form with multiple FileItems. This is ' +
      ...
      if (haveUploadFields)
        this.encoding = isc.JGFormItemView.MULTIPART_ENCODING

      for (let i = 0; i < sectionItems.length; i++) {
        let sectionItem = sectionItems[i],
          isVisible = sectionItem.sectionExpanded

        if (
          isVisible &&
          (!mutexSections || this._lastExpandedSection == sectionItem)
        ) {
      ...
      // comment by xiedh 2014-02-20 框架会设置tabIndex，这里无需更新
    ...
    createItem: function (item, type) {
      ...
      if (
        item.form != null &&
        !(item.form == this.getID() || item.form != this)
      ) {
        this.logWarn(
          "Unsupported 'form' property [" +
            item.form +
            '] set on item:' +
            item +
            '.  Ignoring.'
      ...
      if (item.destroyed && isc.isA.FormItem(item)) {
        this.logWarn(
          'destroyed FormItem passed to setItems()/addItem(): FormItems cannot be ' +
            're-used with different DynamicForms'
        )
      }

      // convert from a simple object into a FormItem
      let className = isc.FormItemFactory.getItemClassName(item, type, this),
        classObject = isc.FormItemFactory.getItemClass(className)

      if (!classObject) {
        this.logWarn(
          'Problem initializing item: ' +
            isc.Log.echo(item) +
            ' - derived FormItem class is: ' +
            className +
      ...
      if (isc.SGWTFactory && isc.isA.SGWTFactory(classObject)) {
        ...
        item = classObject.create(config)
        ...
        // jut as it should.
        return this.createItem(item, item.editorType)
      }
      ...
      if (
        ...
        this.logWarn(
          item.getClass() +
            " form item defined with no '" +
            this.fieldIdProperty +
            "' property - Value will not be saved." +
    ...
    addField: function (field, position) {
      this.addItems(field, position)
    },
    removeField: function (field) {
      this.removeItems(field)
    },
    ...
    _canFocus: function (a, b, c, d) {
      ...
      return this.invokeSuper(
        isc.JGFormItemView,
        isc.JGFormItemView.getInnerPrototypeMethodName('_canFocus'),
        a,
        b,
        c,
        d
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        if (ti != null && ti != -1) {
          ...
          if (explicitTabIndexArray[ti] != null && !warnedTIs[ti]) {
            this.logWarn(
              "More than one item in this form have an explicitly specified tabIndex of '" +
                ti +
                "'. Tab order cannot be guaranteed within this form."
            )
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        if (!isc.isA.FormItem(item)) {
          if (this.logIsDebugEnabled())
            this.logDebug(
              '_assignTabIndices() fired before all form items have been initialized' +
                this.getStackTrace()
            )
    ...
    getTabIndexSpan: function () {
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        let tabIndex = item.tabIndex || item._localTabIndex
        if (tabIndex == null) {
          this._assignTabIndices()
        ...
        if (isc.isA.CanvasItem(item)) {
          let canvas = item.canvas,
            canvasTISpan = 0
          if (canvas && canvas.getTabIndexSpan)
            canvasTISpan = canvas.getTabIndexSpan()
    ...
    _setTabIndex: function () {
      this.Super(
        isc.JGFormItemView.getInnerPrototypeMethodName('_setTabIndex'),
        arguments
      )
      if (this.items) {
    ...
    // Whenever this JGFormItemView is moved, notify all the items that they have been moved.

    handleMoved: function (a, b, c, d) {
      this.invokeSuper(isc.JGFormItemView, 'handleMoved', a, b, c, d)
      this.itemsMoved()
    },

    handleParentMoved: function (a, b, c, d) {
      this.invokeSuper(isc.JGFormItemView, 'handleParentMoved', a, b, c, d)
      this.itemsMoved()
    },

    // Also notify the items if the zIndex is modified
    zIndexChanged: function (a, b, c, d) {
      this.invokeSuper(isc.JGFormItemView, 'zIndexChanged', a, b, c, d)
      this.itemsZIndexChanged()
    },

    parentZIndexChanged: function (a, b, c, d) {
      this.invokeSuper(isc.JGFormItemView, 'parentZIndexChanged', a, b, c, d)
      this.itemsZIndexChanged()
    },
    ...
    scrollTo: function (left, top, reason) {
      let oldLeft = this.getScrollLeft(),
      ...
      if (oldLeft != this.getScrollLeft() || oldTop != this.getScrollTop())
        this.itemsMoved()
    },
    ...
    setValues: function (newData, initTime) {
      ...
      if (isc.isAn.Array(newData)) {
        let useFirst = isc.isA.Object(newData[0])
        this.logWarn(
          'values specified as an array.' +
            (useFirst
              ? ' Treating the first item in the array as intended values.'
      ...
      if (newData == null) {
        ...
        let clonedData = {}
        isc.JGFormItemView._duplicateValues(this, newData, clonedData)
        newData = clonedData
      }
      ...
      this.rememberValues(initTime) // edit by xiedh 2014-02-21 access the initTime to rememberValues
      ...
      if (this.isDrawn()) {
        //condition by xiedh 2014-02-21
        this.markForRedraw('setItemValue')
      }
    },
    ...
    setData: function (newData) {
      ...
      if (data == null) {
        this.values = data = {}
        let isEmpty = this._isEmptyObj(newData)
        if (isEmpty || newData == null) return
      }
      let hasDiff = false
      ...
      if (isc.isA.emptyObject(newData) && !isc.isA.emptyObject(data)) {
        hasDiff = true
      }
      if (hasDiff) {
        this._$newData = newData
    ...
    //          set programmatically via a call to +link{JGFormItemView.setValues()}.
    //
    //      @visibility external
    //		@group formValues
    //
    ...
    rememberValues: function (initTime) {
      //add by xiedh 2014-02-21
      if (initTime === true) {
        this._oldValues = {}
      ...
      // directly.
      isc.JGFormItemView._duplicateValues(
        this,
        values,
        oldVals,
        rememberedDefault
    ...
    //
    // Same as +link{method:JGFormItemView.reset()}.
    //
    // @group formValues
    // @visibility external
    //<
    ...
    resetValues: function () {
      ...
      let values = {}
      isc.JGFormItemView._duplicateValues(this, this._oldValues, values)
      // clear any remembered defaults so they get re-eval'd
      for (let i = 0; i < this._rememberedDefaults; i++) {
        isc.JGFormItemView._clearFieldValue(
          this._rememberedDefaults[i],
          values,
          this
        )
      }
    ...
    clearValues: function () {
      ...
      let data = this.getData()
      if (data) {
        let isEmpty = isc.isA.emptyObject(data)
        if (isEmpty) {
          return
        }
      }
    ...
    valuesHaveChanged: function (returnChangedVals, values, oldValues) {
      ...
      return isc.JGFormItemView.valuesHaveChanged(
        this,
        returnChangedVals,
        values,
        oldValues
    ...
    // Returns all values within this JGFormItemView that have changed since
    ...
    //  saved in this JGFormItemView, call item.updateValue().
    //<
    updateFocusItemValue: function () {
      let focusItem = this.getFocusSubItem()
      if (
        !this._setValuesPending &&
        focusItem != null &&
        focusItem._itemValueIsDirty()
      ) {
    ...
    // <var class="smartclient">+link{CanvasItem.getCriterion()}</var>
    // <var class="smartgwt"><code>CanvasItem.setCriterionGetter()</code></var> for details.
    // <P>
    ...
    // <var class="smartclient">+link{formItem.getCriterion()}</var>
    // <var class="smartgwt"><code>FormItem.setCriterionGetter()</code></var>
    ...
    getValuesAsCriteria: function (advanced, textMatchStyle, returnNulls) {
      ...
      if (!advanced) {
        let values = this._getMappedCriteriaValues()
      ...
      // on top of it.
      let baseCriteria = this._extraAdvancedCriteria
        ? isc.clone(this._extraAdvancedCriteria)
        : {
      ...
      let criteria = this._getMappedCriteriaValues(true, textMatchStyle)
      ...
      let result = isc.DS.checkEmptyCriteria(baseCriteria)
      return result
    },
    ...
    _getMappedCriteriaValues: function (advanced, textMatchStyle) {
      ...
      let values = isc.addProperties({}, this.getValues()),
        simpleCriteria = {},
        advancedCriteria = []
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        if (!advanced) {
          ...
          let criterion = item.getCriterion(textMatchStyle)
      ...
      if (!advanced) {
        ...
        for (let fieldName in values) {
          ...
          advancedCriteria.add({
            ...
            operator: isc.DataSource.getCriteriaOperator(
              values[fieldName],
              textMatchStyle
            ),
            fieldName: fieldName,
    ...
    removeOtherFieldCriteria: function (fieldNames, criteria) {
      let advFields = []
      isc.DS._getAdvancedCriteriaFields(criteria, advFields)
      for (let i = 0; i < advFields.length; i++) {
        let fieldName = advFields[i]
        if (!fieldNames.contains(fieldName)) {
          this.removeFieldCriteria(fieldName, criteria)
        }
      }
    },

    removeFieldCriteria: function (fieldName, criteria) {
      if (!criteria || !criteria.criteria) return false

      for (let i = criteria.criteria.length - 1; i >= 0; i--) {
        if (criteria.criteria[i].criteria) {
          this.removeFieldCriteria(fieldName, criteria.criteria[i])
          if (criteria.criteria[i].criteria.length == 0)
            criteria.criteria.removeAt(i)
        } else {
          if (criteria.criteria[i].fieldName == fieldName)
            criteria.criteria.removeAt(i)
        }
      }
      //return criteria;
    ...
    //  <var class="smartclient">+link{formItem.setCriterion()}</var>
    //  <var class="smartgwt">the <code>FormItemCriterionSetter</code>'s <code>setCriterion()</code> method</var>.
    //  Note that these methods may be overridden for custom
    //  handling. Also note that the default <var class="smartclient">+link{CanvasItem.setCriterion()} implementation</var>
    //  <var class="smartgwt"><code>FormItemCriterionSetter.setCriterion()</code> implementation
    //  used by +link{CanvasItem}</var> handles editing nested criteria via embedded dynamicForms.</li>
    ...
    setValuesAsCriteria: function (criteria, advanced, dropExtraCriteria) {
      if (!advanced && !isc.DataSource.isAdvancedCriteria(criteria)) {
        ...
        if (dropExtraCriteria) delete this._extraAdvancedCriteria
          ...
          let fieldNames = criteria ? isc.getKeys(criteria) : []
          if (isc.getKeys(criteria).length == 0) {
            // no criteria, clear out saved _extraAdvancedCriteria, it no longer applies
            delete this._extraAdvancedCriteria
          } else {
            // remove any _extraAdvancedCriteria entries for fields not in the passed criteria
            this.removeOtherFieldCriteria(
              fieldNames,
              this._extraAdvancedCriteria
            )
            let eAC = this._extraAdvancedCriteria
            if (!eAC || !eAC.criteria || eAC.criteria.length == 0) {
              delete this._extraAdvancedCriteria
            }
          }
        }
        ...
        for (let i = 0; i < items.length; i++) {
          let item = items[i],
            ...
            itemModified = false
          if (
            isc.propertyDefined(criteria, itemName) &&
            item.canEditSimpleCriterion(itemName)
          ) {
            item.setSimpleCriterion(criteria[itemName], itemName)
            itemModified = true
          } else {
            for (let fieldName in criteria) {
              if (
                fieldName != itemName &&
                item.canEditSimpleCriterion(fieldName)
              ) {
        ...
        let topOperator = criteria.operator
        if (topOperator != this.operator) {
          ...
          this.logInfo(
            'Dynamic Form editing advanced criteria object:' +
              isc.Comm.serialize(criteria) +
              ". Form level operator specified as '" +
              this.operator +
              "' - Criteria returned from this form will be nested in an outer " +
              this.operator +
              ' clause.',
            'AdvancedCriteria'
          )
        ...
        for (let i = 0; i < innerCriteria.length; i++) {
          for (let ii = 0; ii < items.length; ii++) {
            ...
            if (this.shouldApplyCriterionToItem(items[ii], innerCriteria[i])) {
              ...
              //                          "to item:" + items[ii]);
              let itemID = items[ii].getID()
              if (assigned[itemID] == null) {
                assigned[itemID] = innerCriteria[i]
                itemsToClear[itemID] = false
              } else {
                let existingCriteria = assigned[itemID]
                let compositeCriterion = isc.DataSource.combineCriteria(
                  existingCriteria,
                  innerCriteria[i],
                ...
                if (!item.canEditCriterion(compositeCriterion)) {
                  this.logInfo(
                    'setValuesAsCriteria(): criteria include:' +
                      this.echoFull(existingCriteria) +
                      ' and ' +
                      this.echoFull(innerCriteria[i]) +
                      '. Both of these ' +
                      'could be applied to item:' +
                      item +
                      '. However, the item is unable to edit a composite criterion ' +
                      'resulting from combining these criteria. Therefore ' +
                      this.echoFull(innerCriteria[i]) +
                      ' will not be applied to this item',
                    'AdvancedCriteria'
                  )
                  ...
                  this.logDebug(
                    'setValuesAsCriteria(): Combined multiple criteria into ' +
                      'composite criterion:' +
                      this.echoFull(compositeCriterion) +
    ...
    shouldApplyCriterionToItem: function (item, criterion) {
      ...
      if (
        criterion.fieldName != null &&
        criterion.fieldName == item.getCriteriaFieldName()
      ) {
        ...
        this.logInfo(
          'Editing AdvancedCriteria in a dynamicForm. Criteria ' +
            'includes a value for field:' +
            criterion.fieldName +
            '. This form includes an item ' +
            item +
            ' with the same fieldName' +
            " but the specified operator '" +
            criterion.operator +
            "' does not match the operator for this form item:" +
            item.getOperator() +
            '. Original criterion will be retained and combined with any ' +
            'criterion returned from this item.',
          'AdvancedCriteria'
        )
      }
      return false
    ...
    getItem: function (itemName, isFieldName) {
      ...
      let item = isc.Class.getArrayItem(
        itemName,
        this.items,
        this.fieldIdProperty
      )
      ...
      let targetDataPath = isc.JGFormItemView._trimDataPath(itemName, this)
      for (let i = 0; i < this.items.length; i++) {
        let path = this.items[i].dataPath
        path = isc.JGFormItemView._trimDataPath(path, this)
    ...
    getValue: function (fieldName) {
      let item = this.getItem(fieldName)
      if (item) {
        let fieldName = item.getTrimmedDataPath() || item.name
      }
      return this._getValue(fieldName)
    },

    _getValue: function (fieldName) {
      return isc.JGFormItemView._getFieldValue(
        fieldName,
        null,
        this.values,
        this,
    ...
    setValue: function (fieldName, value, updatingDisplayValue) {
      let item = this.getFieldItem(fieldName, updatingDisplayValue)
      ...
      if (item != null) {
        ...
        if (!this.storeAtomicValues) {
          let type = item.type ? isc.SimpleType.getType(item.type) : null
    ...
    clearValue: function (fieldName) {
      ...
      if (item != null) item.clearValue()
        ...
        isc.JGFormItemView._clearFieldValue(fieldName, this.values)
    },
    ...
    saveItemValue: function (item, value) {
      ...
      let dataFieldID = item.getDataPath() || item.getFieldName()
      ...
      if (
        this._useDisplayFieldValue(item) &&
        item.displayField != dataFieldID
      ) {
        let displayValue = item.mapValueToDisplay(value)
    ...
    _saveValue: function (field, value, isAtomicValue) {
        ...
        if (!isc.isA.String(fieldName)) {
          ...
          if (isAtomicValue) {
            ...
            if (field == null) {
              ...
              if (ds)
                field =
                  ds.getField(fieldName) || ds.getFieldForDataPath(fieldName)
            }
          }
        }
      }
      isc.JGFormItemView._saveFieldValue(
        fieldName,
        field,
        value,
        this.values,
      ...
      // with the VM values
      let selComponent = this.selectionComponent
      if (!selComponent && this.valuesManager != null) {
        ...
        if (
          isc.isA.ValuesManager(this.valuesManager) &&
          this.valuesManager.members &&
    ...
    clearItemValue: function (item) {
      let fieldName = isc.JGFormItemView._combineDataPaths(
        this.dataPath,
        item.getDataPath() || item.getFieldName()
      )
      isc.JGFormItemView._clearFieldValue(fieldName, this.values)
      if (
    ...
    _saveValues: function (values) {
      ...
      if (
        ...
        let oldFields = isc.getKeys(this.values)
        for (let i in values) {
    ...
    getForm: function (form) {
      let args = form == null ? [this.getFormID()] : arguments
    ...
    getSerializeableFields: function (removeFields, keepFields) {
      ...
      return this.Super(
        'getSerializeableFields',
        [removeFields, keepFields],
        arguments
      )
    },
    ...
    //> @attr JGFormItemView.canTabToSectionHeaders (boolean : null : IRA)
    // If true, the headers for any +link{SectionItems} will be included in the page's tab
    ...
    expandSection: function (sectionID) {
      let section = this.getItem(sectionID)
    ...
    _sectionExpanding: function (section) {
      ...
      if (
        this.sectionVisibilityMode == 'mutex' &&
        this._lastExpandedSection &&
    ...
    getDataPathErrors: function (dataPath) {
      let elements = dataPath.split('/')
      let work = this.errors
    ...
    //      +link{JGFormItemView.showErrors()}<br>
    ...
    setErrors: function (errors, showErrors) {
      this.errors = isc.JGFormItemView.formatValidationErrors(errors)
    ...
    //  Use +link{JGFormItemView.addFieldErrors()} or +link{JGFormItemView.setFieldErrors()} instead
    //<
    setError: function (itemName, errorMessage) {
      let oldError = this.errors[itemName]
      if (!oldError) this.errors[itemName] = errorMessage
      else {
        if (isc.isA.String(oldError))
          this.errors[itemName] = [oldError, errorMessage]
    ...
    // +link{JGFormItemView.showFieldErrors()} to display the errors for this field.
    ...
    // explicit call to +link{JGFormItemView.showFieldErrors()} will display the errors for this field.
    // @param fieldName (string) field to apply the new errors to
    // @param errors (string | array of strings) errors to apply to the field in question
    // @param show (boolean) If true this method will fall through to +link{dynamicForm.showFieldErrors}
    // to update the display
    ...
    //	Clears all errors for this JGFormItemView.
    ...
    draw: function (a, b, c, d) {
      ...
      this.invokeSuper(isc.JGFormItemView, this._$draw, a, b, c, d)

      // We've now written all our items into the DOM - notify them that they are drawn!
      this._itemsDrawn()

      let shouldFocus = this.autoFocus,
        functionName = !shouldFocus
          ? this._$_delayedSetValues
          : this._$_delayedSetValuesFocus
    ...
    _adjustOverflowForPageLoad: function () {
      if (isc.Browser.isSafari) {
        ...
        if (this.isDrawn() && items) {
          for (let i = 0; i < items.length; i++) {
            // 兼容处理原型工具中打开页面，报 _updateHTMLForPageLoad 方法不存在
            items[i]._updateHTMLForPageLoad && items[i]._updateHTMLForPageLoad()
      ...
      return this.Super(
        isc.JGFormItemView.getInnerPrototypeMethodName(
          '_adjustOverflowForPageLoad'
        ),
        arguments
      )
    },
    ...
    //<
    redraw: function () {
      // edit by xiedh 2014-02-13
      /*var reason = this.getRedrawReason();
            if(reason=="setItemValue"){
                this._dirty = false;
                return;
            }*/
      ...
      if (this.couldRedraw()) {
        ...
        //TODO 注意：此属性为内部属性，升级sc版本时需要对应更改
        this._dirty = this.$q7 = false
      }
      this.clearRedrawReason()
      //end
      ...
      let scrollLeft, scrollTop, clipHandle
    ...
    _itemsRedrawn: function () {
      ...
      this.destroyOrphanedItems(
        'Delayed destroy of removed items on form redraw'
      )
    },
    ...
    getElementValues: function () {
      ...
      for (let i = 0; i < this.items.length; i++) {
        let item = this.items[i],
          value = item.getDataElement()
            ? item.getDataElement().value
            : '[no element]'
    ...
    setItemValues: function (values, onRedraw, initTime, items) {
      ...
      let extraCriteria
      if (initTime) {
        extraCriteria = this._extraAdvancedCriteria
          ? this._extraAdvancedCriteria.criteria
          : null
      }
      ...
      for (let itemNum = 0; itemNum < items.length; itemNum++) {
        let item = items[itemNum],
          ...
          //dataPath = item.getTrimmedDataPath(), comment by xiedh 2014-02-21
        ...
        if (values != null && !initTime && !isc.isAn.emptyObject(values)) {
          // add condition(&& !initTime &&!isc.isAn.emptyObject(values)) by xiedh 2014-02-21
          /*if (dataPath) {// comment by xiedh 2014-02-21
                    ...
                    //    if (nestedValues != null) value = nestedValues[segments.last()];
                        value = isc.JGFormItemView._getFieldValue(dataPath, 
                                    (this.storeAtomicValues ? null : item), values, this, true);
                    } else */
          if (fieldName) {
            value = isc.JGFormItemView._getFieldValue(
              fieldName,
              this.storeAtomicValues ? null : item,
              values,
              this,
              true
            )
        ...
        let undefined,
          isUndefined = !fieldName || value === undefined //((!fieldName && !dataPath) || value === undefined); edit by xiedh 2014-02-21
        ...
        let setToCriterion = null
        if (isUndefined && extraCriteria != null) {
          for (let i = 0; i < extraCriteria.length; i++) {
            if (item.canEditCriterion(extraCriteria[i])) {
              ...
              if (setToCriterion == null) {
                ...
                if (!item.canEditCriterion(compositeCriterion)) {
                  this.logInfo(
                    'setItemValues(): current values include multiple extra criteria ' +
                      'that could be applied to form item:' +
                      item +
                      '. Criteria include:' +
                      this.echoFull(setToCriterion) +
                      ' and ' +
                  ...
                  this.logInfo(
                    "setItemValues(): Combined multiple 'extra' criteria into " +
                      'composite criterion:' +
                      this.echoFull(compositeCriterion) +
                      ' and assigned to item:' +
                      item,
              ...
              // "extra" subcriterion to this item by combining them as a composite crit
              //break;
            }
          }
        }
        ...
        if (item.shouldSaveValue == false) {
          if (!isUndefined) {
            ...
            //>DEBUG
            this.logInfo(
              "JGFormItemView.setValues() passed a value for '" +
                item[this.fieldIdProperty] +
                "'." +
            ...
            // add by xiedh 2014-02-14
            if (isSetToDefault && initValue == null) {
              //            		continue;
            }
            //end
            let oldItemValue = isSetToDefault ? null : item._value
        ...
        if (
          isUndefined ||
          (setToCriterion == null && setToExisting && isSetToDefault)
        ) {
          let undef
    ...
    setColWidths: function (colWidths) {
      ...
      if (isc.isA.String(colWidths)) {
        let colWidthsArray = colWidths.split(/[, ]+/)
        ...
        isc.isAn.Array(colWidths) &&
        colWidths.length == 1 &&
        isc.isA.String(colWidths[0])
      ) {
        let colWidthsArray = colWidths[0].split(/[, ]+/)
    ...
    getInnerHTML: function (printCallback) {
      ...
      for (let itemNum = 0; itemNum < items.length; itemNum++) {
        ...
        if (visible && this.isPrinting) {
          ...
          if (item.shouldPrint != null) {
            ...
            let omitControls = this.currentPrintProperties.omitControls
            for (let i = 0; i < omitControls.length; i++) {
              let cName = omitControls[i]
      ...
      if (visibilityChanged || this._itemsChanged)
        isc.Canvas.invalidateTableResizePolicy(items)
      ...
      if (this.flattenItems) {
        ...
        for (let itemNum = 0; itemNum < items.length; itemNum++) {
          ...
          // increment the total columns
          if (item.showTitle && item.titleOrientation != 'top') flatCols++
      ...
      let output = isc.StringBuffer.create()

      // start the form tag
      if (this.writeFormTag && !this.isPrinting)
        output.append(this.getFormTagStartHTML())
      ...
      if (this.titleWidth == this._$star && !this.colWidths) {
        this.colWidths = []
        for (let i = 0; i < this.numCols; i++) this.colWidths[i] = this._$star
      }
      ...
      if (this.colWidths) {
        ...
        if (colWidths.length > this.numCols) {
          if (!this._suppressColWidthWarnings) {
            this.logWarn(
              'colWidths Array longer than numCols, using only first ' +
                this.numCols +
                ' column widths'
            )
          }
          ...
          if (!this._suppressColWidthWarnings) {
            this.logWarn(
              "colWidths Array shorter than numCols, remaining columns get '*' size"
            )
          }
          for (let i = colWidths.length; i < this.numCols; i++)
            colWidths[i] = isc.star
        }
      } else {
        ...
        let totalWidth = this.getInnerContentWidth()
        ...
        let titleCols = Math.floor(this.numCols / 2),
        ...
        if (this.isPrinting) {
          ...
          // the JGFormItemView size -- the printHTML may be written into a
          // different sized container
          elementColWidth = '*'
        } else {
          elementColWidth = Math.floor(
            totalElementColWidth / (this.numCols - titleCols)
          )
          // don't let it get too small
          elementColWidth = Math.max(this.minColWidth, elementColWidth)
        }

        for (let i = 0; i < titleCols; i++) {
        ...
        if (this.logIsInfoEnabled(this._$tablePolicy)) {
          this.logInfo(
            'totalWidth: ' + totalWidth + ', generated colWidths: ' + colWidths,
            this._$tablePolicy
          )
        }
      }
      ...
      let innerWidth = this.getInnerContentWidth(),
        ...
        for (let i = 0; i < items.length; i++) {
          ...
          if (isc.isA.CanvasItem(item) && item.checkCanvasOverflow()) {
            if (!overflowed && this.logIsInfoEnabled(this._$tablePolicy)) {
              this.logInfo(
                'CanvasItem: ' + item + ' overflowed, rerunning policy',
                this._$tablePolicy
              )
            }
            overflowed = true
      ...
      for (let colNum = 0; colNum < colWidths.length; colNum++) {
        let colWidth = colWidths[colNum]
      ...
      let topRowCellStart = isc.JGFormItemView._getTopRowCellStart()
      for (let colNum = 0; colNum < colWidths.length; colNum++) {
        if (!isc.isA.Number(colWidths[colNum])) {
          output.append(
            topRowCellStart.join(isc.emptyString),
            this._$topRowCellEnd
          )
        } else {
          let innerWidth = colWidths[colNum]
          ...
          topRowCellStart[3] = isc.FormItem
            ? isc.FormItem.getPrototype().baseStyle
            : null

          let spacerHeight = isc.Browser.isIE ? 1 : 0,
            cellStart = topRowCellStart.join(isc.emptyString)
          output.append(
            cellStart,
            this.fixedColWidths
              ? isc.Canvas.spacerHTML(innerWidth, spacerHeight)
              : null,
            this._$topRowCellEnd
          )
        }
      ...
      if (this.autoSendTarget && this.target)
        output.append(this._getAutoSendTargetHTML())
      ...
      let completeInnerHTMLFun = function completeInnerHTMLFun(htmlOutputs) {
        ...
        if (self.writeFormTag && !self.isPrinting)
          output.append(self._$tableFormClose)
        // end just the table
        else output.append(self._$tableClose)

        let HTML = output.release()
        if (wentAsync) {
      ...
      if (len > 0) {
        ...
        let includeInNextCell = [],
          htmlOutputs = new Array(len),
          completedCount = 0

        let itemCompletedFun = function itemCompletedFun() {
        ...
        let theHTML
        for (let itemNum = 0; itemNum < len; ++itemNum) {
          let item = items[itemNum],
          ...
          //>DEBUG
          if (this.logIsDebugEnabled())
            this.logDebug('Drawing FormItem: ' + item) //<DEBUG
          ...
          if (item._startRow || itemNum == 0) {
            if (itemNum != 0) itemOutput.append(this._$rowEnd)
            if (item._emptyRows > 0) {
              for (let i = 0; i < item._emptyRows; i++)
                itemOutput.append(this._$rowStart, this._$rowEnd)
            }
            itemOutput.append(this._$rowStart)
            if (item._emptyCells > 0) {
              for (let i = 0; i < item._emptyCells; i++)
          ...
          let completeIncludedInnerHTMLFun = (function (
            itemNum,
            item,
            itemOutput,
            visible,
            ...
            let func = function func(HTML) {
              ...
              if (
                showErrors &&
                item.getErrorOrientation() == isc.Canvas.BOTTOM
              ) {
            ...
            return function (includedHtmlOutputs) {
              if (includedHtmlOutputs != null) {
                ...
                let itemOutputStream = itemOutput.getArray()
                itemOutputStream.push.apply(
                  itemOutputStream,
                  includedHtmlOutputs
                )
              }
              ...
              if (visible) {
                ...
                if (self.isPrinting) {
                  let printHTML = item.getPrintHTML(
                    self.currentPrintProperties,
                    func
                  )
                  if (printHTML == null) {
                    return false
                  ...
                  return func(
                    item.getInnerHTML(value, true, self.showInlineErrors)
                  )
                }
          ...
          let includedLen = includeInNextCell.length
          if (includedLen > 0) {
            let includedHtmlOutputs = new Array(includedLen)

            let includedCompletedFun = (function (
              completeIncludedInnerHTMLFun,
              includedLen,
              includedHtmlOutputs
            ) {
              let includedCompletedCount = 0
              return function () {
            ...
            for (let m = 0; m < includedLen; ++m) {
              let includedItem = includeInNextCell[m]
              ...
              if (this.isPrinting) {
                let printHTML = includedItem.getPrintHTML(
                  self.currentPrintProperties,
                  innerFunc
                )
                if (printHTML == null) {
                ...
                theHTML = innerFunc(
                  includedItem.getInnerHTML(includedItem.getValue())
                )
              }
            }
    ...
    createErrorItem: function () {
      let errorItem = isc.addProperties(
        {
          cellStyle: this.errorItemCellStyle
        },
    ...
    //> @method JGFormItemView.getErrorsHTML()
    ...
    getErrorsHTML: function (errors) {
      ...
      let errorString = this.errorsPreamble
      errorString += '<UL>'
      for (let field in errors) {
        ...
        errorString +=
          '<LI>' + (item ? item.getTitle() : field) + ' : ' + message + '</LI>'
      }
      errorString += '</UL>'
    ...
    _$autoSendTargetTemplate: [
      "<INPUT TYPE=HIDDEN NAME='", // target field name
      ,
      "' VALUE='", // target
      ,
      "'>"
    ...
    getCellStartHTML: function (item, error) {
      ...
      if (colSpan == '*') {
        let startCol = item._tablePlacement ? item._tablePlacement[0] : 0
        colSpan = this.numCols - startCol
      }

      let className = item.getCellStyle()

      // Use the height calculated by tableResizePolicy rather than the specified size (may be
      // null, "*" or a percentage).

      let forceHeight = this.fixedRowHeights || item.shouldFixRowHeight()
      let height = item._size ? item._size[1] : null

      if (isc.isA.Number(height) && this.cellSpacing != 0)
        height -= 2 * this.cellSpacing
      if (
        isc.Browser.isStrict &&
        isc.isA.Number(height) &&
        this.cellPadding != 0
      ) {
      ...
      return this._getCellStartHTML(
        item.align
          ? item.align
          : (this.form ? this.form.isRTL() : this.isRTL())
          ? isc.Canvas.RIGHT
          : isc.Canvas.LEFT,
        item.vAlign,
    ...
    _getCellStartHTML: function (
      ...
      if (itemID && formID) {
        output.append(isc.JGFormItemView._containsItem, "='", itemID, "'")
      }
    ...
    getTitleOrientation: function (item) {
      ...
      return (
        (item ? item.titleOrientation : null) ||
    ...
    getTitleAlign: function (item) {
      let form = this.form || this // for ContainerItem method-stealing hack
      return item.titleAlign
        ? item.titleAlign
        : this.titleAlign
        ? this.titleAlign
    ...
    getTitleSpanHTML: function (item, error) {
      ...
      output.append(
        '<SPAN ',
        this._containsItemTitleAttrHTML(item),
        " CLASS='",
        item.getTitleStyle(),
        "' ALIGN=",
        this.getTitleAlign(item),
        '>'
      )
    ...
    _$widthColon: 'width:',
    _$maxWidthColon: 'max-width:',
    _$maxHeightColon: 'max-height:',
    _$heightColon: 'height:',
    _$NOBR: '<NOBR>',
    _$innerTitleTableClose: '</td></tr></TABLE>',
    _$divClose: '</DIV>',
    _$tdClose: '</TD>',

    _outerTitleCellTemplate: [
      '<TD ', // 0 // 1: this._containsItemTitleAttrHTML(item)
      ,
      " CLASS='", // 2 // 3: className
      ,
      "' ALIGN='", // 4 // 5: this.getTitleAlign(item)
      ,
      "' VALIGN='", // 6 // 7: this.getTitleVAlign(item)
      ,
      "'", // 8: // 9: possible rowspan // 10: possible colspan
      ,
      ,
    ...
    getTitleCellHTML: function (item, error) {
      ...
      let cellTemplate = this._outerTitleCellTemplate
      ...
      if (item.getRowSpan() > 1)
        cellTemplate[9] = ' ROWSPAN=' + item.getRowSpan()
      else cellTemplate[9] = null
      if (item.getTitleColSpan() > 1)
        cellTemplate[10] = ' COLSPAN=' + item.getTitleColSpan()
    ...
    getTitleCellInnerHTML: function (item, error) {
      ...
      let heightProperty = isc.Browser.isMoz
          ? this._$maxHeightColon
          : this._$heightColon,
        widthProperty = isc.Browser.isMoz
          ? this._$maxWidthColon
          : this._$widthColon
      ...
      if (clipTitle) {
        if (this._titleClipDivTemplate == null) {
          this._titleClipDivTemplate = [
            ...
            'white-space:nowrap;', // 1 // 2: possible width
            ,
            "' ", // 3
            isc.JGFormItemView._itemPart, // 4
            "='", // 5
            isc.JGFormItemView._title, // 6
            "' ", // 7
            isc.JGFormItemView._containsItem, // 8
            "='", // 9 // 10: item ID
            ,
            "'>" // 11
          ]
          if (this.emitOuterTextOverflow) {
            this._titleClipDivTemplate[0] +=
              isc.Browser._textOverflowPropertyName + ':ellipsis;'
          }
        }

        let divTemplate = this._titleClipDivTemplate

        if (titleWidth != null)
          divTemplate[2] = widthProperty + titleWidth + 'px;'
    ...
    _containsItemTitleAttrHTML: function (item) {
      if (!isc.JGFormItemView._itemTitleAttrHTML) {
        isc.JGFormItemView._itemTitleElementAttrHTML = [
          ' ',
          isc.JGFormItemView._containsItem,
          "='",
          null, // item ID
          "' ",
          isc.JGFormItemView._itemPart,
          "='",
          isc.JGFormItemView._title,
          "' ",
      ...
      isc.JGFormItemView._itemTitleElementAttrHTML[3] = item.getItemID()
      // [Item ID is unique]
      isc.JGFormItemView._itemTitleElementAttrHTML[10] =
        this._getTitleCellID(item)
      return isc.JGFormItemView._itemTitleElementAttrHTML.join(isc.emptyString)
    },
    ...
    updateTitleCellState: function (item) {
      let titleCell = this.getTitleCell(item)
    ...
    titleClipped: function (item) {
      let titleClipper = this._getTitleClipper(item)
      return (
        titleClipper != null &&
    ...
    _titleClipperTemplate: [
      ...
      "<div style='float:right'>", // 1 // 2: common title suffix
      ,
      "</div><div id='", // 3 // 4: "titleClipper" DOM ID
      ,
      "' style='overflow:hidden;", // 5
      isc.Browser._textOverflowPropertyName, // 6
      ':ellipsis', // 7
      isc.Browser.isIE && !isc.Browser.isStrict ? ';width:100%' : '', // 8
      "'>", // 9 (note that white-space:nowrap is inherited) // 10 & 11: common title prefix, title HTML
      ,
      ,
      '</div>', // 12
    ...
    getTitleHTML: function (item, error, clipTitle) {
      ...
      let title = item.visible ? item.getTitleHTML() : null
      if (title) {
        let required = this.isRequired(item),
          orientation = this.getTitleOrientation(item),
          leftPrefix =
            orientation == isc.Canvas.LEFT || orientation == isc.Canvas.TOP

        if (clipTitle) {
          let clipperTemplate = this._titleClipperTemplate

          if (required && this.hiliteRequiredFields) {
            if (leftPrefix) {
              let exclusiveRequiredTitlePrefix =
                  this.exclusiveRequiredTitlePrefix,
              ...
              if (exclusiveRequiredTitlePrefix == null) {
                if (this.requiredTitlePrefix.endsWith(this.titlePrefix)) {
                  exclusiveRequiredTitlePrefix =
                    this.requiredTitlePrefix.substring(
                      0,
                      this.requiredTitlePrefix.length - this.titlePrefix.length
                    )
                } else {
              ...
              if (exclusiveRequiredTitleSuffix == null) {
                if (this.requiredTitleSuffix.startsWith(this.titleSuffix)) {
                  exclusiveRequiredTitleSuffix =
                    this.requiredTitleSuffix.substring(this.titleSuffix.length)
                } else {
              ...
              clipperTemplate[2] = this.requiredTitleSuffix.substring(
                0,
                this.requiredTitleSuffix.length -
                  exclusiveRequiredTitleSuffix.length
              )
              clipperTemplate[4] = this._getTitleClipperID(item)
              clipperTemplate[10] = this.requiredTitlePrefix.substring(
                exclusiveRequiredTitlePrefix.length
              )
              clipperTemplate[11] = title
              clipperTemplate[13] = exclusiveRequiredTitleSuffix
            } else {
              let exclusiveRequiredRightTitlePrefix =
                  this.exclusiveRequiredRightTitlePrefix,
                exclusiveRequiredRightTitleSuffix =
                  this.exclusiveRequiredRightTitleSuffix
              if (exclusiveRequiredRightTitlePrefix == null) {
                if (
                  this.requiredRightTitlePrefix.endsWith(this.rightTitlePrefix)
                ) {
                  exclusiveRequiredRightTitlePrefix =
                    this.requiredRightTitlePrefix.substring(
                      0,
                      this.requiredRightTitlePrefix.length -
                        this.rightTitlePrefix.length
                    )
                } else {
                  exclusiveRequiredRightTitlePrefix =
                    this.requiredRightTitlePrefix
                }
              }
              if (exclusiveRequiredRightTitleSuffix == null) {
                if (
                  this.requiredRightTitleSuffix.startsWith(
                    this.rightTitleSuffix
                  )
                ) {
                  exclusiveRequiredRightTitleSuffix =
                    this.requiredRightTitleSuffix.substring(
                      this.rightTitleSuffix.length
                    )
                } else {
                  exclusiveRequiredRightTitleSuffix =
                    this.requiredRightTitleSuffix
                }
              }

              clipperTemplate[0] = exclusiveRequiredRightTitlePrefix
              clipperTemplate[2] = this.requiredRightTitleSuffix.substring(
                0,
                this.requiredRightTitleSuffix.length -
                  exclusiveRequiredRightTitleSuffix.length
              )
              clipperTemplate[4] = this._getTitleClipperID(item)
              clipperTemplate[10] = this.requiredRightTitlePrefix.substring(
                exclusiveRequiredRightTitlePrefix.length
              )
              clipperTemplate[11] = title
            ...
            if (leftPrefix) {
              ...
              clipperTemplate[2] = this.titleSuffix.substring(
                0,
                this.titleSuffix.length - this.exclusiveTitleSuffix.length
              )
              clipperTemplate[4] = this._getTitleClipperID(item)
              clipperTemplate[10] = this.titlePrefix.substring(
                this.exclusiveTitlePrefix.length
              )
              clipperTemplate[11] = title
              clipperTemplate[13] = this.exclusiveTitleSuffix
            } else {
              clipperTemplate[0] = this.exclusiveRightTitlePrefix
              clipperTemplate[2] = this.rightTitleSuffix.substring(
                0,
                this.rightTitleSuffix.length -
                  this.exclusiveRightTitleSuffix.length
              )
              clipperTemplate[4] = this._getTitleClipperID(item)
              clipperTemplate[10] = this.rightTitlePrefix.substring(
                this.exclusiveRightTitlePrefix.length
              )
              clipperTemplate[11] = title
    ...
    _$formTagStartTemplate: [
      '<FORM ', // 0
      'ID', // 1
      '=', // 2 // 3: this.getFormID() // 4: absolute positioning, or null
      ,
      ,
      ' METHOD=', // 5 // 6: this.method
      ,
      " ACTION='", // 7 // 8: this.action
      ,
      "' ENCTYPE=", // 9 // 10: multipart or normal encoding // 11: Target= or null // 12: target or null // 13: close target quote or null
      ,
      ,
      ,
      ,
      " ONSUBMIT='return ", // 14 // 15: this.getID()
      ,
      "._handleNativeSubmit()' ONRESET='", // 16 // 17: this.getID()
      ,
    ...
    getFormTagStartHTML: function () {
      let template = this._$formTagStartTemplate,
      ...
      if (this.isMultipart())
        template[10] = isc.JGFormItemView.MULTIPART_ENCODING
      else template[10] = isc.JGFormItemView.NORMAL_ENCODING
    ...
    _$tableStartTemplate: [
      "<TABLE role='presentation' ID='", // 0 // 1:  this._getTableElementID()
      ,
      "' WIDTH='", // 2 // 3: innerContentWidth / innerWidth
      ,
      "' CELLSPACING='", // 4 // 5: this.cellSpacing
      ,
      "' CELLPADDING='", // 6 // 7: this.cellPadding
      ,
      "' BORDER='", // 8 // 9: this.cellBorder
      ,
    ...
    getTableStartHTML: function () {
      ...
      let isForm = isc.isA.JGFormItemView(this),
        template = isForm
          ? this._$tableStartTemplate
          : isc.JGFormItemView.getPrototype()._$tableStartTemplate
      template[1] = this._getTableElementID()
      if (this.isPrinting) {
        template[2] = isForm
          ? this._$widthEquals
          : isc.JGFormItemView.getPrototype()._$widthEquals
        template[3] = '100%'
        ...
        template[2] = isForm
          ...
          : isc.JGFormItemView.getPrototype()._$widthEquals
        template[3] =
          this.getInnerContentWidth != null
    ...
    layoutChildren: function (a, b, c, d) {
      this.invokeSuper(isc.JGFormItemView, 'layoutChildren', a, b, c, d)
      let items = this.getItems()
      if (!items) return
      for (let i = 0; i < items.length; i++) {
        // redraw for any percent sized / "*" width child
        let width = items[i].width,
          height = items[i].height
        if (
          (isc.isA.String(width) &&
            (width.contains('%') || width.contains('*'))) ||
          (isc.isA.String(height) &&
    ...
    getAbsPosHTML: function () {
      let output = isc.SB.create()
      // for each item in the list, get HTML output for it and combine the output
      for (let itemNum = 0, len = this.items.length; itemNum < len; itemNum++) {
        ...
        let includeHint = !item._getShowHintInField(),
          includeErrors = this.showInlineErrors
        output.append(
          item.getStandaloneItemHTML(
            item.getValue(),
            includeHint,
            includeErrors
          )
    ...
    getScrollWidth: function (recalculate) {
      ...
      for (let i = 0; i < this.items.length; i++) {
        ...
        let handle = item.getAbsDiv()
        if (handle) {
          let itemRight = handle.scrollWidth + item._getPercentCoord(item.left)
    ...
    getScrollHeight: function (recalculate) {
      ...
      for (let i = 0; i < this.items.length; i++) {
        ...
        if (handle) {
          let itemBottom =
            handle.scrollHeight + item._getPercentCoord(item.top, true)
    ...
    //      +link{JGFormItemView.canSubmit} must be set to <code>true</code></li>
    ...
    submitForm: function () {
      if (!this._formWillSubmit()) {
        this.logWarn(
          'Attempt to perform direct submission on JGFormItemView where this.canSubmit ' +
      ...
      if (this.getFileItemForm() != null) {
        this.logWarn(
          'Performing a direct submission on a JGFormItemView containing a FileItem. ' +
      ...
      try {
        ...
        this.logWarn(
          'Form submission was unsuccessful. In some browsers this can occur when ' +
    ...
    performImplicitSave: function (item, onPause) {
      ...
      if (item) {
        if (item._shouldUpdateParentItem || item.$887) {
          //TODO 压缩版兼容
          item.parentItem.updateValue()
        }
        if (item.$da != null) isc.Timer.clear(item.$da) //TODO 压缩版兼容
        if (item._fireOnPauseTimer != null)
          isc.Timer.clear(item._fireOnPauseTimer)
      }

      if (this.awaitingImplicitSave) delete this.awaitingImplicitSave
      this.logInfo(
        'performImplicitSave called ' +
          (!onPause
            ? 'by editorExit()'
            : 'after implicitSaveDelay (' + this.implicitSaveDelay + 'ms)') +
          ' for item ' +
          item.name +
          '.'
      )
      ...
      if (this.valuesManager) {
        ...
        this.valuesManager.saveData(
          this.getID() + '._implicitSaveCallback(data)',
          {
            showPrompt: false
          }
        )
      } else {
        this.saveData(this.getID() + '._implicitSaveCallback(data)', {
          showPrompt: false
        })
      }
    },
    ...
    _addItemToImplicitSaveUpdateArray: function (item) {
      let storage = this.valuesManager ? this.valuesManager : this
    ...
    _implicitSaveCallback: function (data) {
      ...
      if (storage.itemsToUpdateState) {
        for (let i = 0; i < storage.itemsToUpdateState.length; i++) {
          let item = storage.itemsToUpdateState[i]
    ...
    //> @attr JGFormItemView.formSubmitFailedWarning (String : "Form was unable to be submitted. The most likely cause for this is an invalid value in an upload field." : IRWA)
    ...
    formSubmitFailedWarning:
      'Form was unable to be submitted. The most likely cause for this is an ' +
      'invalid value in an upload field.',

    //> @method JGFormItemView.formSubmitFailed() [A]
    ...
    formSubmitFailed: function () {
      ...
      let transactionText = this.getValues()._transaction
      if (transactionText != null && isc.RPCManager && isc.XMLTools) {
        let doc = isc.XMLTools.parseXML(this.getValues()._transaction),
          transactionNum
        if (doc)
          transactionNum = isc.XMLTools.selectNumber(doc, '//transactionNum')
        if (transactionNum != null) {
          isc.RPCManager.doClearPrompt(transactionNum)
          isc.RPCManager.clearTransaction(transactionNum)
        }

        let transactionItem = this.getItem('_transaction')
    ...
    //> @method JGFormItemView.setAction()
    // Sets the +link{JGFormItemView.action,action} for this form.
    ...
    //> @method JGFormItemView.setTarget()
    // Sets the +link{JGFormItemView.target,target} for this form.
    ...
    //> @method JGFormItemView.setMethod()
    // Sets the +link{JGFormItemView.method,method} for this form.
    ...
    getFileItemForm: function () {
      ...
      for (let i = 0; i < items.length; i++) {
        if (isc.isA.FileItem(items[i])) {
          let fileItemCanvas = items[i].canvas
          // Make sure that the FileItem's canvas is a JGFormItemView before returning it because
          // there are cases where the canvas is not a form (for example, if the FileItem is
          // read-only).
          if (isc.isA.JGFormItemView(fileItemCanvas)) return fileItemCanvas
        }
      }
      return null
    },
    ...
    // _handleNativeSubmit.
    // This method is fired from the onsubmit handler for the HTML form for this JGFormItemView widget.
    ...
    // +link{JGFormItemView.handleHiddenValidationErrors()}
    // <P>
    ...
    validate: function (
      ...
      if (
        this.dataSource &&
        this.dataSource.useLocalValidators != null &&
        this.useLocalValidators == false
      )
        return true

      let // remember if we had errors before
        hadErrorsBefore = this.hasErrors(),
      ...
      let errors = {},
        ...
        dsFields =
          validateHiddenFields && !ignoreDSFields && this.dataSource
      ...
      if (this.valuesManager != null) {
        ...
        if (this.dataPath != null) {
          record = isc.JGFormItemView._getFieldValue(
            this.dataPath,
            null,
            record,
            this,
      ...
      let validationOptions = {
        unknownErrorMessage: this.unknownErrorMessage,
      ...
      let wasAlreadyQueuing = isc.rpc ? isc.rpc.startQueue() : false

      for (let itemNum = 0; itemNum < this.items.length; itemNum++) {
        ...
        if (item.validators != null) {
          ...
          let fieldResult = this.validateField(
            item,
            item.validators,
            value,
            record,
          ...
          if (fieldResult != null) {
            if (fieldResult.errors != null) {
              fieldErrorsFound = this.addValidationError(
                errors,
                column || dp,
                fieldResult.errors
              )
        ...
        if (hidden && fieldErrorsFound)
          hiddenErrors[column || dp] = errors[column || dp]
      ...
      // our specific fields
      if (this.rulesEngine != null) {
        let rulesErrors = this.rulesEngine.applyFieldValidators(errors, this)
      ...
      if (dsFields) {
        ...
        for (let i in dsFields) {
          ...
          if (errors[fieldName] != null)
            hiddenErrors[fieldName] = errors[fieldName]
        }
      }
      ...
      //>DEBUG
      if (errorsFound)
        this.logInfo('Validation errors: ' + isc.Log.echoAll(errors))
      //<DEBUG
    ...
    //> @method JGFormItemView.valuesAreValid()
    ...
    // <P>
    // Unlike +link{JGFormItemView.validate()} this method will not store the errors on the JGFormItemView
    ...
    valuesAreValid: function (validateHiddenFields, returnErrors) {
      let errors = this.validate(validateHiddenFields, null, null, true)
    ...
    //> @method JGFormItemView.getValidatedValues()
    ...
    //> @method JGFormItemView.showErrors()
    ...
    // +link{JGFormItemView.handleHiddenValidationErrors(), handleHiddenValidationErrors()} to
    ...
    showErrors: function (errors, hiddenErrors) {
      let suppressAutoFocus =
        !this.autoFocusOnError || this._suppressAutoFocusOnErrors
      if (this._suppressAutoFocusOnErrors)
        delete this._suppressAutoFocusOnErrors
      ...
      //comment by xiedh 2014-03-19 校验不通过，焦点无法离开
    ...
    //> @method JGFormItemView.showFieldErrors ()
    ...
    isRequired: function (item) {
      return (
        isc.JGFormItemView.canEditField(item, this) &&
    ...
    setRequiredIf: function () {
      ...
      for (let itemNum = 0; itemNum < this.items.length; itemNum++) {
        ...
        for (let v = 0; v < validators.length; v++) {
          ...
          let type = isc.Validator.getValidatorType(validator)
          if (type == this._$requiredIf) {
            ...
            if (
              ...
              !isc.isA.Function(validator.expression)
            ) {
            ...
            // expression
            //TODO 压缩版兼容
            item.$11m = item._required = validator.expression.apply(this, [
              item,
              validator,
              value
            ])
    ...
    setFocus: function (hasFocus) {
      ...
      if (hasFocus) {
        ...
        let event = isc.EH.lastEvent
        if (
          item != null &&
          !item.hasFocus /*!(event.target == this && event.eventType == isc.EH.MOUSE_DOWN)*/
        ) {
          // edit by xiedh 2014-02-28
    ...
    _focusInNextTabElement: function (forward, mask, skipItems, item) {
      if (
        ...
        this.logInfo(
          'JGFormItemView - focusInNextTabElement() running. Delegating to Super()',
          'syntheticTabIndex'
        )
        return this.Super(
          isc.JGFormItemView.getInnerPrototypeMethodName(
      ...
      if (item == null) {
        this.logInfo(
          'JGFormItemView - focusInNextTabElement() running. Focusing at end.',
          'syntheticTabIndex'
        )
      ...
      while (item.parentItem) {
        if (item._moveFocusWithinItem(forward)) {
          this.logInfo(
            'JGFormItemView - focusInNextTabElement() - allowed:' +
              item +
      ...
      if (item._moveFocusWithinItem(forward)) {
        this.logInfo(
          'JGFormItemView - focusInNextTabElement() running. allowed:' +
            item +
            ' to shift focus internally.',
          'syntheticTabIndex'
        )
      ...
      this.logInfo(
        'JGFormItemView - focusInNextTabElement() moving to next item:' +
          item +
          ', forward?' +
          forward,
        'syntheticTabIndex'
    ...
    _getNextFocusItem: function (item, forward) {
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        let gti = otherItem.getGlobalTabIndex()
        if (gti < 0) {
          continue
        }
        ...
        if (forward) {
          ...
          if (
            (gti < currentTabIndex || (gti == currentTabIndex && index > i)) &&
            (nextTabIndex == null || nextTabIndex <= gti)
          ) {
            nextItem = otherItem
            nextTabIndex = gti
    ...
    focusAtEnd: function (start) {
      let startItem = this._getStartItemForFocusAtEnd(start)
    ...
    focusInItem: function (itemName, focusAtEnd) {
      ...
      if (item._canFocus()) {
        // 处理当文本框为只读时，禁用文本值规范格式
        if (this.isReadOnly() && item.mask) item.mask = null
        else if (item.mask == null) item.mask = item.maskBackUp
        ...
        if (this._setValuesPending) {
          ...
          isc.Page.setEvent(
            'idle',
            function () {
              if (!theForm.destroyed) theForm.focusInItem()
            },
            isc.Page.FIRE_ONCE
          )
        }
    ...
    _focusInItemWithoutHandler: function (item) {
      ...
      let hasFocus = item.hasFocus
      if (isc.Browser.isIE) {
        let focusItemInfo = isc.JGFormItemView._getItemInfoFromElement(
    ...
    clearingElement: function (item) {
      if (
        this.__suppressFocusHandler != null &&
        this.__suppressFocusItem == item
      ) {
      ...
      if (
        this.__suppressBlurHandler != null &&
        this.getFocusSubItem() == item
      ) {
    ...
    _focusChanged: function (hasFocus) {
      this.Super(
        isc.JGFormItemView.getInnerPrototypeMethodName('_focusChanged'),
        arguments
      )
    ...
    parentVisibilityChanged: function (newVisibility) {
      ...
      if (!this.isVisible() && isc.Browser.isMoz)
        this._blurItem(this.getFocusSubItem())
    ...
    _allowNativeTextSelection: function (event) {
      let itemInfo = this._getEventTargetItemInfo(event)
      // For now always allow text selection of form items' cells.
      if (itemInfo.item) {
        let rv = itemInfo.item._allowNativeTextSelection(event, itemInfo)
    ...
    prepareForDragging: function (a, b, c, d) {
      ...
      let event = EH.lastEvent,
        itemInfo = this._getEventTargetItemInfo(event)
      if (
        itemInfo.item &&
        (itemInfo.overElement ||
          itemInfo.overTextBox ||
    ...
    // then allow the event to be fired on the JGFormItemView, and bubbled up through the widget
    // parent chain.
    ...
    _getEventTargetItemInfo: function (event) {
      ...
      let target = isc.EH.isMouseEvent(event.eventType)
        ? event.nativeTarget
        : event.nativeKeyTarget
      let info = isc.JGFormItemView._getItemInfoFromElement(target, this)
    ...
    // than over the title or within the form item's cell in the JGFormItemView but outside the
    // text box area.
    // @visibility external
    //<
    ...
    getEventItemInfo: function () {
      let itemInfo = this._getEventTargetItemInfo()
    ...
    handleMouseStillDown: function (event, eventInfo) {
      ...
      let targetInfo = this._getEventTargetItemInfo(event),
        item =
          targetInfo.overTitle || targetInfo.inactiveContext
            ? null
            : targetInfo.item

      // avoid double delivery of events if there are nested JGFormItemView elements all receiving
    ...
    handleMouseDown: function (event, eventInfo) {
      ...
      if (item != null) {
        ...
        if (
          ...
          !targetInfo.inactiveContext &&
          targetInfo.overElement &&
    ...
    _itemMouseEvent: function (itemInfo, eventType) {
      let lastMoveItem = this._lastMoveItem,
      ...
      if (
        ...
        item &&
        item.hasDataElement() &&
        (item._dataElementIsTextBox || item.$15i)
      ) {
        //TODO 压缩版处理
    ...
    handleMouseOver: function (event, eventInfo) {
      if (this.mouseOver && this.mouseOver(event, eventInfo) == false)
        return false
      let result = this._itemMouseEvent(
        this._getEventTargetItemInfo(event),
        isc.EH.MOUSE_OVER
      )
      return result
    },
    ...
    handleMouseMove: function (event, eventInfo) {
      ...
      if (this.mouseMove && this.mouseMove(event, eventInfo) == false)
        return false
      let result = this._itemMouseEvent(
        this._getEventTargetItemInfo(event),
        isc.EH.MOUSE_MOVE
    ...
    handleMouseOut: function (event, eventInfo) {
      ...
      // if appropriate)
      if (this.mouseOut && this.mouseOut(event, eventInfo) == false)
        return false
    },
    ...
    bubbleItemHandler: function (itemID, handlerName, arg1, arg2, arg3, arg4) {
      let subItem = this.getItemById(itemID),
        result = null
      ...
      for (; subItem != null; subItem = subItem.parentItem) {
        ...
        if (
          subItem[handlerName] != null &&
          !isc.isA.Function(subItem[handlerName])
        ) {
          isc.Func.replaceWithMethod(
            subItem,
            handlerName,
        ...
        if (subItem[handlerName] == null) {
          this.logWarn(
            'handler:' + handlerName + ' is not present on itemID ' + itemID
          )
          return false
        }
    ...
    bubbleInactiveEditorEvent: function (item, eventName, itemInfo) {
      return this.bubbleItemHandler(
        item,
        '_handleInactiveEditorEvent',
        eventName,
        itemInfo.inactiveContext,
        itemInfo
      )
    },
    ...
    elementChanged: function (itemID) {
      ...
      let result = this.bubbleItemHandler(itemID, 'elementChanged', itemID)
      return result != false
    },
    ...
    handleClick: function (event, eventInfo) {
      ...
      if (this.editingOn) {
        if (
          ...
          this.logWarn(
            'No item clicked upon, passing the click to the DF',
            'EventHandler'
          )
      ...
      if (itemInfo && itemInfo.item) {
        ...
        let mouseDownInfo = this._mouseDownTarget || {},
          mouseDownItem = this._mouseDownTarget
            ? this._mouseDownTarget.item
            : null
        if (mouseDownItem == itemInfo.item) {
          if (itemInfo.inactiveContext) {
            this.logInfo(
              'Bubbling inactive editor event for ' + item.ID,
              'EventHandler'
            )
            ...
            if (this._mouseDownTarget.overTitle && itemInfo.overTitle) {
              this.logInfo(
                'Bubbling handleTitleClick event for ' + item.ID,
                'EventHandler'
              )
              ...
              let isSpacer = item.isA('SpacerItem'),
                overItem =
                  isSpacer ||
                  itemInfo.overElement ||
                  itemInfo.overTextBox ||
                ...
                wasOverItem =
                  isSpacer ||
                  mouseDownInfo.overElement ||
                  mouseDownInfo.overTextBox ||
                  mouseDownInfo.overControlTable

              if (
                mouseDownInfo.overIcon &&
                itemInfo.overIcon &&
                item.form == this
              ) {
                ...
                let icon = item.getIcon(itemInfo.overIcon)
              ...
              if (overItem && wasOverItem) {
                this.logInfo(
                  'Bubbling handleClick event for ' + item.ID,
                  'EventHandler'
                )
                if (
                  this.bubbleItemHandler(item, 'handleClick', item) == false
                ) {
                  returnVal = false
                }
              }
              ...
              if (returnVal != false) {
                ...
                // false).
                this.logInfo(
                  'Bubbling handleCellClick event for ' + item.ID,
                  'EventHandler'
                )
                returnVal = this.bubbleItemHandler(
                  item,
                  'handleCellClick',
                  item
                )
      ...
      if (returnVal == false || returnVal == isc.EH.STOP_BUBBLING)
        return returnVal
    ...
    handleDoubleClick: function (event, eventInfo) {
      ...
      if (
        ...
        itemInfo.item &&
        mouseDownInfo &&
        mouseDownInfo.item == itemInfo.item
      ) {
        let item = itemInfo.item
        if (itemInfo.inactiveContext) {
          returnVal = this.bubbleInactiveEditorEvent(
            item,
            'doubleClick',
            itemInfo
          )
          ...
          if (overItem && wasOverItem) {
            if (
              this.bubbleItemHandler(item, 'handleDoubleClick', item) == false
            ) {
              returnVal = false
            }
          }
    ...
    elementBlur: function (element, itemID) {
      ...
      let result = true
      if (this.__suppressBlurHandler == null)
        result = this.bubbleItemHandler(itemID, 'elementBlur')
      else {
    ...
    handleKeyPress: function (event, eventInfo) {
      ...
      if (
        ...
        !isc.JGFormItemView.canEditField(this.getFocusSubItem(), this)
      ) {
        return false
      }
      ...
      if (
        event.characterValue != null &&
        event.characterValue != 0 &&
    ...
    //  @visibility external
    //<
    itemHoverHTML: isc.JGFormItemView._defaultItemHoverHTMLImpl,
    ...
    //  @visibility external
    //<
    valueHoverHTML: isc.JGFormItemView._defaultValueHoverHTMLImpl,
    ...
    _showItemHover: function (item, HTML) {
      if (HTML && !isc.is.emptyString(HTML) && item.showHover != false) {
        let properties = this._getHoverProperties(item)
    ...
    _getHoverProperties: function (item) {
      ...
      if (item) {
        props = isc.addProperties(
          ...
          {
            align:
              item.hoverAlign != null ? item.hoverAlign : this.itemHoverAlign,
            hoverDelay:
              item.hoverDelay != null ? item.hoverDelay : this.itemHoverDelay,
            height:
              item.hoverHeight != null
                ? item.hoverHeight
                : this.itemHoverHeight,
            opacity:
              item.hoverOpacity != null
                ? item.hoverOpacity
                : this.itemHoverOpacity,
            baseStyle:
              item.hoverStyle != null ? item.hoverStyle : this.itemHoverStyle,
            showHover: item.showHover != null ? item.showHover : this.showHover,
            valign:
              item.hoverVAlign != null
                ? item.hoverVAlign
                : this.itemHoverVAlign,
            width:
              item.hoverWidth != null ? item.hoverWidth : this.itemHoverWidth,
    ...
    isMultipart: function () {
      ...
      return !(
        this.encoding == isc.JGFormItemView.NORMAL ||
        this.encoding == isc.JGFormItemView.NORMAL_ENCODING
      )
    },
    ...
    itemIsLastInRow: function (item, rowNum) {
      let rowTable = this.items._rowTable,
        row = rowTable[rowNum],
    ...
    getColumnWidths: function () {
      ...
      for (let j = 0; j < widths.length; j++) widths[j] = 0

      for (let rowCount = 0; rowCount < rowTable.length; rowCount++) {
        let row = rowTable[rowCount]
        for (let i = 0; i < row.length; i++) {
          let item = this.items.get(row[i])
    ...
    getItemTableOffsets: function (item, overrideRowTable) {
      let rowTable = overrideRowTable || this.items._rowTable,
      ...
      for (let rowCount = 0; rowCount < rowTable.length; rowCount++) {
        ...
        if (start > -1 && end > -1) {
          ...
          if (!result.width || result.width < end - start)
    ...
    getItemDropIndex: function (item, dropSide) {
      ...
      let offsets = this.getItemTableOffsets(item),
      ...
      if (dropSide == 'T') {
        ...
        return this.getItemIndexAtTableLocation(
          offsets.top - (offsets.top == 0 ? 0 : 1),
          offsets.left
        )
      }
      if (dropSide == 'B') {
        let bottom = offsets.top + offsets.height - 1
        let itemIndex = this.getItemIndexAtTableLocation(
          bottom + 1,
          offsets.left
        )
        if (itemIndex == null) {
    ...
    getItemAtPageOffset: function (x, y) {
      ...
      let colNum = this.inWhichPosition(widths, x - this.getPageLeft()),
      ...
      let itemIndex = rowTable[rowNum][colNum],
    ...
    getNearestItem: function (x, y) {
      ...
      this.logDebug(
        'Computing nearest item to (' + x + ',' + y + ')',
        'formItemDragDrop'
      )

      for (let i = 0; i < this.items.length; i++) {
        let item = this.items[i]
        let // "true" = return a rect including the title
          area = item.getPageRect(true),
          left = area[0],
          top = area[1],
          width = area[2],
          height = area[3],
    ...
    showDragLineForItem: function (item, mouseX, mouseY) {
      ...
      if (item.showTitle != false) {
        if (titlesAt == 'left' || titlesAt == 'right')
          width += item.getVisibleTitleWidth()
      ...
      // smaller
      let sideExtent = width / 4
      if (sideExtent > 20) sideExtent = 20

      if (mouseY <= top) mouseY = top + 1
      else if (mouseY >= top + height) mouseY = top + height - 1

      let lOffset = mouseX - left,
        lPercent = Math.round(width / lOffset),
        tOffset = mouseY - top,
        tPercent = Math.round(height / tOffset),
        rOffset = left + width - mouseX,
        rPercent = Math.round(width / rOffset),
        bOffset = top + height - mouseY,
        bPercent = Math.round(height / bOffset),
        side = 'R',
        lineHeight,
        lineWidth,
        lineLeft,
      ...
      if (
        toRight ||
        (Math.min(lPercent, rPercent) < Math.min(tPercent, bPercent) &&
      ...
      if (
        this.itemIsLastInRow(item, item._dragRowNum) &&
        !this.canAddColumns &&
        item.dropSide == 'R'
      ) {
    ...
    adjustDragLinePosition: function (dims, item, side) {
      ...
      if (side == 'T') {
        if (row == 0) return
        if (
          rowTable[row - 1][colFrom] == rowTable[row - 1][colTo] &&
          rowTable[row - 1][colFrom - 1] != rowTable[row - 1][colFrom] &&
          rowTable[row - 1][colTo + 1] != rowTable[row - 1][colFrom]
        ) {
          let rect = this.items[rowTable[row - 1][colFrom]].getPageRect(true)
          let otherY = rect[1] + rect[3]
      ...
      if (side == 'L') {
        ...
        let rect = this.items[rowTable[row][colFrom - 1]].getPageRect(true)
        let otherX = rect[0] + rect[2]
      ...
      if (side == 'R') {
        ...
        // Need support for row-spanning columns here
        let rect = this.items[rowTable[row][colTo + 1]].getPageRect(true)
        let otherX = rect[0]
    ...
    getSelectionChain: function () {
      ...
      for (let i = selComponents.length - 1; i >= 0; i--) {
        indices.add(
          selComponents[i].getRecordIndex(selComponents[i].getSelectedRecord())
        )
      }
      return indices
    },
    ...
    //
    // @see attr:JGFormItemView.readOnlyDisplay
    ...
    // @visibility internal
    //<
  }) // END isc.JGFormItemView.addMethods()

  // class methods
  isc.JGFormItemView.addClassMethods({
    ...
    _$select: 'select',
    _$checkbox: 'checkbox',
    _$staticText: 'staticText',
    _$boolean: 'boolean',
    _$integer: 'integer',
    _$binary: 'binary',
    _$blob: 'blob',
    _$multifile: 'multifile',
    _$multiupload: 'multiupload',
    _$upload: 'upload',
    _$file: 'file',
    _$base64Binary: 'base64Binary',
    _$enum: 'enum',
    _$CycleItem: 'CycleItem',
    _$selectOther: 'selectOther',
    _$relation: 'relation',
    _$nestedEditor: 'NestedEditorItem',
    _$nestedListEditor: 'NestedListEditorItem',
    _$imageFile: 'imageFile',
    _$viewFileItem: 'ViewFileItem',
    _$section: 'section',
    _$sectionItem: 'SectionItem',
    _$button: 'button',
    _$buttonItem: 'ButtonItem',
    ...
    canEditField: function (field, widget) {
      ...
      if (widget && widget.canEditField) {
        ...
        this.logDebug(
          'Component ' +
            widget +
            " calling 'canEditField()' method for field:" +
            field.name,
          'canEditField'
        )
        //<DEBUG
    ...
    _getItemInfoFromElement: function (target, form) {
      let handle = form ? form.getClipHandle() : document,
        itemInfo = {},
        containsItem = isc.JGFormItemView._containsItem,
        itemPart = isc.JGFormItemView._itemPart,
        elementString = isc.JGFormItemView._element,
        textBoxString = isc.JGFormItemView._textBoxString,
        controlTableString = isc.JGFormItemView._controlTableString,
        inlineErrorString = isc.JGFormItemView._inlineErrorString,
        titleString = isc.JGFormItemView._title
      ...
      while (target && target != handle && target != document) {
        let itemID = target.getAttribute
          ? target.getAttribute(containsItem)
          : null
        if (itemID != null && !isc.isAn.emptyString(itemID)) {
          ...
          if (item && !item.destroyed) {
            ...
            let inactiveContext = item._getInactiveContextFromElement(target)
            if (inactiveContext != null) {
              if (this.logIsDebugEnabled('inactiveEditorHTML')) {
                this.logDebug(
                  'Event occurred over inactive HTML for item:' +
                    item +
                    ' inactiveContext:' +
                    this.echo(inactiveContext),
                  'inactiveEditorHTML'
                )
              }
            ...
            let eventItemPart = target.getAttribute(itemPart)
            ...
            else if (eventItemPart == controlTableString)
              itemInfo.overControlTable = true
            else if (eventItemPart == inlineErrorString)
              itemInfo.overInlineError = true
    ...
    formatValidationErrors: function (errors) {
      ...
      for (let fieldName in errors) {
        ...
        if (isc.isAn.Array(fieldErrors)) {
          ...
          errorMessages[fieldName] = isc.isAn.Object(fieldErrors)
    ...
    compareValues: function (value1, value2) {
      ...
      if (isc.isA.Date(value1) && isc.isA.Date(value2))
        ...
        for (let i = 0; i < value1.length; i++) {
          if (!isc.JGFormItemView.compareValues(value1[i], value2[i]))
            return false
        }
        return true
      } else {
        ...
        if (
          isc.isA.Number(value1) ||
          isc.isA.String(value1) ||
          isc.isA.Boolean(value1)
        ) {
          value1 = value1.valueOf()
        }
        if (
          isc.isA.Number(value2) ||
          isc.isA.String(value2) ||
          isc.isA.Boolean(value2)
        ) {
          value2 = value2.valueOf()
        }
        if (value1 == value2) return true
        if (isc.isAn.Object(value1) && isc.isAn.Object(value2)) {
          let tempObj = isc.addProperties({}, value2)
          for (let attr in value1) {
    ...
    //
    // Implemented as a classMethod and used by JGFormItemView.valuesHaveChanged
    ...
    valuesHaveChanged: function (
      ...
      for (let prop in values) {
        ...
        let fullPath = rootPath == null ? prop : rootPath + '/' + prop
        ...
        let item = form.getItem(fullPath)
        if (item != null) {
          ...
          let valIsObj = isc.isA.Object(value),
          ...
          if (
            oldValIsObj &&
            (isc.isA.Number(oldValue) ||
              isc.isA.String(oldValue) ||
              isc.isA.Boolean(oldValue))
          ) {
          ...
          if (
            ...
            !isc.isA.Date(value) &&
            oldValIsObj &&
            !isc.isAn.Array(oldValue) &&
            !isc.isA.Date(oldValue)
          ) {
            let innerChanged = this.valuesHaveChanged(
              form,
              returnChangedVals,
              values[prop],
              oldValues[prop],
            ...
            changed = !isc.JGFormItemView.compareValues(value, oldValue)
    ...
    _getTitleInnerTableTemplate: function () {
      ...
      if (this._titleInnerTableTemplate == null) {
        this._titleInnerTableTemplate = [
          '<TABLE height=', // 0 // 1: height
          ,
          " border=0 cellspacing=0 cellpadding=0><tr><td class='", // 2 // 3: className
          ,
          // Override any style attributes that would look wrong double-applied by the className
          "' style='" + isc.Canvas._$noStyleDoublingCSS + "' ALIGN='", // 4 // 5: this.getTitleAlign(item)
          ,
          "'>", // 6
          null // 7: <NOBR>
        ]
  ...
  // See QA/JGFormItemView/inlineForms.jsp
  // ---------------------------------------------------------------------------------------

  isc.defineClass('InlineFormItem', 'JGFormItemView').addProperties({
    position: 'relative',
  ...
  isc.InlineFormItem.addClassMethods({
    ...
    create: function (A, B, C, D, E, F, G, H, I, J, K, L, M) {
      let itemProps = isc.addProperties(
        {
          showTitle: false,
          validate: function () {
            this.form.validate()
      ...
      let theForm = this.createRaw().completeCreation(
        {
          fields: [itemProps],
  ...
  isc.JGFormItemView.addClassMethods({
    getInnerPrototypeMethodName: function (methodName) {
      let map = isc.JGFormItemView._prototypeMethodMap
      if (map) {
        if (map.hasOwnProperty(methodName)) {
    ...
    //> @classMethod JGFormItemView.makeInlineItem()
    ...
    //    &lt;script&gt;isc.JGFormItemView.makeInlineItem("title", "comboBox",
    ...
    //> @classMethod JGFormItemView.getFormValues()
  ...
  isc.JGFormItemView.registerStringMethods({
    ...
    // @param	form      (JGFormItemView)   the form being submitted
    // @group submitting
  ...
  isc.JGFormItemView.addClassProperties({
    ...
    _element: '$9b',
    _textBoxString: '$9d',
    _controlTableString: '$9f',
    _inlineErrorString: 'inlineErrorHandle',
    $99r: 'inlineErrorHandle',
    _title: '$9g',
    //实例方法映射
    _prototypeMethodMap: {}
  })
  isc.JGFormItemView.addClassMethods({
    _combineDataPaths: function () {
      return isc.JGFormItemView.$702.apply(this, arguments)
    },

    _clearFieldValue: function () {
      return isc.JGFormItemView.$70m.apply(this, arguments)
    },

    _duplicateValues: function () {
      return isc.JGFormItemView.$91f.apply(this, arguments)
    },

    _getFieldValue: function () {
      return isc.JGFormItemView.$70o.apply(this, arguments)
    },

    _saveFieldValue: function () {
      return isc.JGFormItemView.$70n.apply(this, arguments)
    },

    _trimDataPath: function () {
      return isc.JGFormItemView.$840.apply(this, arguments)
    },
    addPrototypeMethods: function () {
      let prototype = isc.JGFormItemView.getPrototype()
      let map = isc.JGFormItemView._prototypeMethodMap
      for (let name in map) {
        prototype[map[name]] = prototype[name]
      }
    }
  })
  ...
  let prototype = isc.JGFormItemView.getPrototype()
  prototype._getDOMID = prototype.$qs

  /**
   *注释start至end部分在启动服务时会自动被替换,作为锚点勿修改
   */
  //start
  isc.JGFormItemView._prototypeMethodMap = {
    _assignTabIndices: '$10o',
    _saveValues: '$10r',
    _destroyItems: '$10k',
    _addItems: '$10l',
    _delayedSetValuesFocus: '$9p',
    _delayedSetValues: '$9n',
    _getHoverProperties: '$wc',
    _handleNativeSubmit: '$10d',
    _itemsCleared: '$11a',
    _implicitSaveCallback: '$888',
    _notifyCanvasItems: '$11c',
    _focusInItemWithoutHandler: '$11b',
    _saveValue: '$10w',
    _focusInNextTabElement: '$kf',
    _getMappedCriteriaValues: '$76v',
    _getTitleClipper: '$1141',
    _getTitleClipperID: '$1140',
    _getStartItemForFocusAtEnd: '$120d',
    _blurItem: '$11r',
    _formWillSubmit: '$11q',
    _getTableElementID: '$11n',
    _getTableElement: '$11o',
    _readyToSetFocus: '$vl',
    _setTabIndex: '$vk',
    _getCellEndHTML: '$11h',
    _containsItemTitleAttrHTML: '$11i',
    _getAutoSendTargetHTML: '$11f',
    _getCellStartHTML: '$11g',
    _absPos: '$11e',
    _getEventTargetItemInfo: '$ne',
    _itemMouseEvent: '$11y',
    _canAnimateClip: '$1f',
    _adjustOverflowForPageLoad: '$us',
    _handleHiddenValidationErrors: '$21z',
    _canFocusInItem: '$11u',
    _getNextFocusItem: '$11t',
    _propagateOperationsToFileItem: '$114v',
    _sectionExpanding: '$100',
    _itemsDrawn: '$102',
    _sectionCollapsing: '$101',
    _placeCanvasItems: '$103',
    _blurFocusItemWithoutHandler: '$106',
    _itemsRedrawing: '$107',
    _itemsRedrawn: '$108',
    _getValue: '$70p',
    _slotChildrenIntoTabOrder: '$v2',
    _suppressFocusHandlerForItem: '$65n',
    _allowNativeTextSelection: '$kr',
    _canFocus: '$kk',
    _addItemToImplicitSaveUpdateArray: '$95x',
    _useDisplayFieldValue: '$425',
    _getTitleCellID: '$427',
    _showItemHover: '$119',
    _focusChanged: '$lf',
    _saveAtomicValue: '$924'
  }
  isc.JGFormItemView.addPrototypeMethods()
  //end
}
