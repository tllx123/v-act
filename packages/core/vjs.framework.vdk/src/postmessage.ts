...
const postMsg = {
  ...
  isSimpleOrgin: function (iden) {
    if (
      !iden ||
      iden !=
        VMetrix._fn.md5(window.location.protocol + '//' + window.location.host)
    ) {
      return false
    }
    return true
  ...
  init: function (params) {
    ...
    var callback = function (sandbox) {
      ...
      if (scope) {
        ...
        var _parmas = {
          iden: postMsg._getKey(componentCode, windowCode, url),
          componentCode: componentCode,
          windowCode: windowCode,
  ...
  close: function (params) {
    ...
    if (params.closeAll) {
      ...
      var _urlObj = params._$urlObj
      var info = this._getInfo(params.componentCode, params.windowCode, _urlObj)
      if (!info) {
        return
      }
